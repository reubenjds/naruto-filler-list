import type { Episode, Arc, QuickList } from '$lib/index';

export const naruto: Episode[] = [
	{
		number: 1,
		title: 'Enter: Naruto Uzumaki!',
		type: 'Canon'
	},
	{
		number: 2,
		title: 'My Name is Konohamaru!',
		type: 'Canon'
	},
	{
		number: 3,
		title: 'Sasuke and Sakura: Friends or Foes?',
		type: 'Canon'
	},
	{
		number: 4,
		title: 'Pass or Fail: Survival Test',
		type: 'Canon'
	},
	{
		number: 5,
		title: "You Failed! Kakashi's Final Decision",
		type: 'Canon'
	},
	{
		number: 6,
		title: 'A Dangerous Mission! Journey to the Land of Waves',
		type: 'Canon'
	},
	{
		number: 7,
		title: 'The Assassin of the Mist',
		type: 'Mixed Canon/Filler'
	},
	{
		number: 8,
		title: 'The Oath of Pain',
		type: 'Canon'
	},
	{
		number: 9,
		title: 'Kakashi: Sharingan Warrior',
		type: 'Mixed Canon/Filler'
	},
	{
		number: 10,
		title: 'The Forest of Chakra',
		type: 'Canon'
	},
	{
		number: 11,
		title: 'The Land Where a Hero Once Lived',
		type: 'Canon'
	},
	{
		number: 12,
		title: 'Battle on the Bridge! Zabuza Returns!',
		type: 'Canon'
	},
	{
		number: 13,
		title: "Haku's Secret Jutsu: Crystal Ice Mirrors",
		type: 'Canon'
	},
	{
		number: 14,
		title: 'The Number One Hyperactive, Knucklehead Ninja Joins the Fight',
		type: 'Mixed Canon/Filler'
	},
	{
		number: 15,
		title: 'Zero Visibility: The Sharingan Shatters',
		type: 'Mixed Canon/Filler'
	},
	{
		number: 16,
		title: 'The Broken Seal',
		type: 'Mixed Canon/Filler'
	},
	{
		number: 17,
		title: 'White Past: Hidden Ambition',
		type: 'Canon'
	},
	{
		number: 18,
		title: 'The Weapons Known as Shinobi',
		type: 'Mixed Canon/Filler'
	},
	{
		number: 19,
		title: 'The Demon in the Snow',
		type: 'Mixed Canon/Filler'
	},
	{
		number: 20,
		title: 'A New Chapter Begins: The Chunin Exam!',
		type: 'Mixed Canon/Filler'
	},
	{
		number: 21,
		title: 'Identify Yourself: Powerful New Rivals',
		type: 'Mixed Canon/Filler'
	},
	{
		number: 22,
		title: 'Chunin Challenge: Rock Lee vs. Sasuke!',
		type: 'Canon'
	},
	{
		number: 23,
		title: 'Genin Takedown! All Nine Rookies Face Off!',
		type: 'Mixed Canon/Filler'
	},
	{
		number: 24,
		title: 'Start Your Engines: The Chunin Exam Begins!',
		type: 'Mixed Canon/Filler'
	},
	{
		number: 25,
		title: 'The Tenth Question: All or Nothing',
		type: 'Canon'
	},
	{
		number: 26,
		title: 'Special Report: Live from the Forest of Death',
		type: 'Filler',
		explanation: "Recap episode that summarizes the events of the Chunin Exam's early stages."
	},
	{
		number: 27,
		title: 'The Chunin Exam Stage 2: The Forest of Death',
		type: 'Mixed Canon/Filler'
	},
	{
		number: 28,
		title: 'Eat or Be Eaten: Panic in the Forest',
		type: 'Mixed Canon/Filler'
	},
	{
		number: 29,
		title: "Naruto's Counterattack: Never Give In!",
		type: 'Mixed Canon/Filler'
	},
	{
		number: 30,
		title: 'The Sharingan Revived: Dragon Flame Jutsu',
		type: 'Mixed Canon/Filler'
	},
	{
		number: 31,
		title: "Bushy Brow's Pledge: Undying Love and Protection",
		type: 'Canon'
	},
	{
		number: 32,
		title: 'Sakura Blossoms!',
		type: 'Canon'
	},
	{
		number: 33,
		title: 'Battle Formation: Ino-Shika-Cho!',
		type: 'Canon'
	},
	{
		number: 34,
		title: "Akamaru Trembles: Gaara's Cruel Strength",
		type: 'Canon'
	},
	{
		number: 35,
		title: "The Scroll's Secret: No Peeking Allowed",
		type: 'Canon'
	},
	{
		number: 36,
		title: 'Clone vs. Clone: Mine are Better Than Yours!',
		type: 'Canon'
	},
	{
		number: 37,
		title: 'Surviving the Cut! The Rookie Nine Together Again',
		type: 'Mixed Canon/Filler'
	},
	{
		number: 38,
		title: 'Narrowing the Field: Sudden Death Elimination',
		type: 'Mixed Canon/Filler'
	},
	{
		number: 39,
		title: "Bushy Brow's Jealousy: Lions Barrage Unleashed",
		type: 'Mixed Canon/Filler'
	},
	{
		number: 40,
		title: 'Kakashi and Orochimaru: Face-to-Face!',
		type: 'Mixed Canon/Filler'
	},
	{
		number: 41,
		title: 'Kunoichi Rumble: The Rivals Get Serious!',
		type: 'Mixed Canon/Filler'
	},
	{
		number: 42,
		title: 'The Ultimate Battle: Cha!',
		type: 'Canon'
	},
	{
		number: 43,
		title: 'Killer Kunoichi and a Shaky Shikamaru',
		type: 'Mixed Canon/Filler'
	},
	{
		number: 44,
		title: "Akamaru Unleashed! Who's Top Dog Now?",
		type: 'Mixed Canon/Filler'
	},
	{
		number: 45,
		title: "Surprise Attack! Naruto's Secret Weapon",
		type: 'Mixed Canon/Filler'
	},
	{
		number: 46,
		title: 'Byakugan Battle: Hinata Grows Bold!',
		type: 'Mixed Canon/Filler'
	},
	{
		number: 47,
		title: 'A Failure Stands Tall!',
		type: 'Mixed Canon/Filler'
	},
	{
		number: 48,
		title: 'Gaara vs. Rock Lee: The Power of Youth Explodes!',
		type: 'Canon'
	},
	{
		number: 49,
		title: "Lee's Hidden Strength: Forbidden Secret Jutsu!",
		type: 'Canon'
	},
	{
		number: 50,
		title: 'The Fifth Gate: A Splendid Ninja is Born',
		type: 'Canon'
	},
	{
		number: 51,
		title: 'A Shadow in Darkness: Danger Approaches Sasuke',
		type: 'Canon'
	},
	{
		number: 52,
		title: "Ebisu Returns: Naruto's Toughest Training Yet",
		type: 'Mixed Canon/Filler'
	},
	{
		number: 53,
		title: 'Long Time No See: Jiraiya Returns!',
		type: 'Mixed Canon/Filler'
	},
	{
		number: 54,
		title: 'The Summoning Jutsu: Wisdom of the Toad Sage!',
		type: 'Mixed Canon/Filler'
	},
	{
		number: 55,
		title: 'A Feeling of Yearning, A Flower Full of Hope',
		type: 'Mixed Canon/Filler'
	},
	{
		number: 56,
		title: 'Live or Die: Risk it All to Win it All!',
		type: 'Mixed Canon/Filler'
	},
	{
		number: 57,
		title: 'He Flies! He Jumps! He Lurks! Chief Toad Appears!',
		type: 'Mixed Canon/Filler'
	},
	{
		number: 58,
		title: 'Hospital Besieged: The Evil Hand Revealed!',
		type: 'Mixed Canon/Filler'
	},
	{
		number: 59,
		title: 'The Final Rounds: Rush to the Battle Arena!',
		type: 'Mixed Canon/Filler',
		explanation: 'Mostly filler but has some connections to the main plot.'
	},
	{
		number: 60,
		title: 'Byakugan vs. Shadow Clone',
		type: 'Mixed Canon/Filler'
	},
	{
		number: 61,
		title: 'Ultimate Defense: Zero Blind Spot!',
		type: 'Canon'
	},
	{
		number: 62,
		title: "A Failure's True Power",
		type: 'Canon'
	},
	{
		number: 63,
		title: 'Hit it or Quit it: The Final Rounds Get Complicated!',
		type: 'Mixed Canon/Filler'
	},
	{
		number: 64,
		title: 'Zero Motivation: The Guy with Cloud Envy',
		type: 'Canon'
	},
	{
		number: 65,
		title: 'Dancing Leaf, Squirming Sand',
		type: 'Canon'
	},
	{
		number: 66,
		title: "Bushy Brow's Jutsu: Sasuke Style!",
		type: 'Mixed Canon/Filler'
	},
	{
		number: 67,
		title: 'Late for the Show, But Ready to Go! The Ultimate Secret Technique is Born',
		type: 'Canon'
	},
	{
		number: 68,
		title: 'Zero Hour! The Destruction of the Hidden Leaf Village Begins!',
		type: 'Canon'
	},
	{
		number: 69,
		title: 'Village in Distress: A New A-Ranked Mission!',
		type: 'Mixed Canon/Filler'
	},
	{
		number: 70,
		title: "A Shirker's Call to Action: A Layabout No More!",
		type: 'Mixed Canon/Filler'
	},
	{
		number: 71,
		title: 'An Unrivaled Match: Hokage Battle Royale!',
		type: 'Mixed Canon/Filler'
	},
	{
		number: 72,
		title: 'A Mistake from the Past: A Face Revealed!',
		type: 'Mixed Canon/Filler'
	},
	{
		number: 73,
		title: 'Forbidden Secret Technique: Reaper Death Seal!',
		type: 'Canon'
	},
	{
		number: 74,
		title: "Astonishing Truth! Gaara's Identity Emerges!",
		type: 'Mixed Canon/Filler'
	},
	{
		number: 75,
		title: "Sasuke's Decision: Pushed to the Edge!",
		type: 'Canon'
	},
	{
		number: 76,
		title: 'Assassin of the Moonlit Night',
		type: 'Canon'
	},
	{
		number: 77,
		title: 'Light vs. Dark: The Two Faces of Gaara',
		type: 'Canon'
	},
	{
		number: 78,
		title: "Naruto's Ninja Handbook",
		type: 'Canon'
	},
	{
		number: 79,
		title: 'Beyond the Limit of Darkness and Light',
		type: 'Canon'
	},
	{
		number: 80,
		title: 'The Third Hokage, Forever...',
		type: 'Canon'
	},
	{
		number: 81,
		title: 'Return of the Morning Mist',
		type: 'Canon'
	},
	{
		number: 82,
		title: 'Eye to Eye: Sharingan vs. Sharingan!',
		type: 'Canon'
	},
	{
		number: 83,
		title: "Jiraiya: Naruto's Potential Disaster!",
		type: 'Mixed Canon/Filler'
	},
	{
		number: 84,
		title: 'Roar, Chidori! Brother vs. Brother!',
		type: 'Canon'
	},
	{
		number: 85,
		title: 'Hate Among the Uchihas: The Last of the Clan!',
		type: 'Canon'
	},
	{
		number: 86,
		title: 'A New Training Begins: I Will Be Strong!',
		type: 'Canon'
	},
	{
		number: 87,
		title: 'Keep on Training: Pop Goes the Water Balloon!',
		type: 'Canon'
	},
	{
		number: 88,
		title: 'Focal Point: The Mark of the Leaf',
		type: 'Canon'
	},
	{
		number: 89,
		title: "An Impossible Choice: The Pain Within Tsunade's Heart",
		type: 'Canon'
	},
	{
		number: 90,
		title: 'Unforgivable! A Total Lack of Respect!',
		type: 'Canon'
	},
	{
		number: 91,
		title: 'Inheritance! The Necklace of Death',
		type: 'Canon'
	},
	{
		number: 92,
		title: "A Dubious Offer! Tsunade's Choice",
		type: 'Canon'
	},
	{
		number: 93,
		title: 'Breakdown! The Deal is Off!',
		type: 'Canon'
	},
	{
		number: 94,
		title: 'Attack! Fury of the Rasengan!',
		type: 'Canon'
	},
	{
		number: 95,
		title: 'The Fifth Hokage! A Life on the Line',
		type: 'Canon'
	},
	{
		number: 96,
		title: 'Deadlock! Sannin Showdown',
		type: 'Canon'
	},
	{
		number: 97,
		title: "Kidnapped! Naruto's Hot Spring Adventure!",
		type: 'Filler',
		explanation:
			'This episode features a comedic plot about Naruto and friends going to a hot spring.'
	},
	{
		number: 98,
		title: "Tsunade's Warning: Ninja No More",
		type: 'Mixed Canon/Filler'
	},
	{
		number: 99,
		title: 'The Will of Fire Still Burns',
		type: 'Mixed Canon/Filler',
		explanation: 'Mostly filler, but has some connections to the main story.'
	},
	{
		number: 100,
		title: 'Sensei and Student: The Bond of the Shinobi',
		type: 'Mixed Canon/Filler'
	},
	{
		number: 101,
		title: "Gotta See! Gotta Know! Kakashi-Sensei's True Face!",
		type: 'Filler',
		explanation:
			"Humorous episode focused on Team 7's attempts to see Kakashi's face. Recommended Watch"
	},
	{
		number: 102,
		title: 'Mission: Help an Old Friend in the Land of Tea',
		type: 'Filler',
		explanation: 'Land of Tea filler arc. You can skip this without missing any main story details.'
	},
	{
		number: 103,
		title: 'The Race is on! Trouble on the High Seas!',
		type: 'Filler',
		explanation:
			'Continuation of the Land of Tea arc. Focuses on a race across the ocean that has no relevance to the main storyline.'
	},
	{
		number: 104,
		title: 'Run Idate Run! The Forged Report!',
		type: 'Filler',
		explanation: 'More of the Land of Tea filler arc.'
	},
	{
		number: 105,
		title: 'A Fierce Battle of Rolling Thunder!',
		type: 'Filler',
		explanation:
			'Land of Tea filler arc, featuring a battle that has no impact on the core plot of the series.'
	},
	{
		number: 106,
		title: 'The Last Leg: A Final Act of Desperation',
		type: 'Filler',
		explanation: 'Concludes the Land of Tea filler arc. Skippable Arc.'
	},
	{
		number: 107,
		title: 'The Battle Begins: Naruto vs. Sasuke',
		type: 'Canon'
	},
	{
		number: 108,
		title: 'Bitter Rivals and Broken Bonds',
		type: 'Canon'
	},
	{
		number: 109,
		title: 'An Invitation from the Sound',
		type: 'Canon'
	},
	{
		number: 110,
		title: 'Formation! The Sasuke Retrieval Squad!',
		type: 'Canon'
	},
	{
		number: 111,
		title: 'Sound vs. Leaf',
		type: 'Canon'
	},
	{
		number: 112,
		title: 'Squad Mutiny: Everything Falls Apart',
		type: 'Mixed Canon/Filler'
	},
	{
		number: 113,
		title: 'Full Throttle Power! Choji, Ablaze!',
		type: 'Canon'
	},
	{
		number: 114,
		title: "Goodbye Old Friend...! I'll Always Believe in You!",
		type: 'Canon'
	},
	{
		number: 115,
		title: 'Your Opponent is Me!',
		type: 'Canon'
	},
	{
		number: 116,
		title: "360 Degrees of Vision: The Byakugan's Blind Spot",
		type: 'Canon'
	},
	{
		number: 117,
		title: 'Losing is Not an Option!',
		type: 'Canon'
	},
	{
		number: 118,
		title: 'The Vessel Arrives Too Late',
		type: 'Canon'
	},
	{
		number: 119,
		title: 'Miscalculation: A New Enemy Appears',
		type: 'Canon'
	},
	{
		number: 120,
		title: 'Roar and Howl! The Ultimate Tag Team!',
		type: 'Canon'
	},
	{
		number: 121,
		title: 'To Each His Own Battle',
		type: 'Canon'
	},
	{
		number: 122,
		title: "Fakeout: Shikamaru's Comeback!",
		type: 'Canon'
	},
	{
		number: 123,
		title: "The Leaf's Handsome Devil!",
		type: 'Canon'
	},
	{
		number: 124,
		title: 'The Beast Within',
		type: 'Canon'
	},
	{
		number: 125,
		title: 'The Sand Shinobi: Allies of the Leaf',
		type: 'Canon'
	},
	{
		number: 126,
		title: 'Showdown: Gaara vs. Kimimaro',
		type: 'Canon'
	},
	{
		number: 127,
		title: 'Vengeful Strike! The Bracken Dance!',
		type: 'Canon'
	},
	{
		number: 128,
		title: 'A Cry on Deaf Ears',
		type: 'Canon'
	},
	{
		number: 129,
		title: 'Brothers: Distance Among the Uchiha',
		type: 'Canon'
	},
	{
		number: 130,
		title: 'Father and Son, the Broken Crest',
		type: 'Mixed Canon/Filler'
	},
	{
		number: 131,
		title: 'The Secrets of the Mangekyō Sharingan!',
		type: 'Mixed Canon/Filler'
	},
	{
		number: 132,
		title: 'For a Friend',
		type: 'Canon'
	},
	{
		number: 133,
		title: 'A Plea from a Friend',
		type: 'Canon'
	},
	{
		number: 134,
		title: 'The End of Tears',
		type: 'Canon'
	},
	{
		number: 135,
		title: 'The Promise That Could Not Be Kept',
		type: 'Canon'
	},
	{
		number: 136,
		title: 'Deep Cover?! A Super S-Ranked Mission',
		type: 'Filler',
		explanation:
			'Starts a filler arc involving a mission that is not part of the main story. It has minor plot elements but can be skipped without losing any crucial details.'
	},
	{
		number: 137,
		title: 'A Town of Outlaws: The Shadow of the Fuma Clan',
		type: 'Filler',
		explanation:
			'Introduces characters and a storyline about the Fuma Clan that is not part of the manga. It is a self-contained filler plot.'
	},
	{
		number: 138,
		title: 'Pure Betrayal, and a Fleeting Plea',
		type: 'Filler',
		explanation:
			'Continuation of the Fuma Clan filler arc. This episode does not tie into any main plot developments.'
	},
	{
		number: 139,
		title: 'Pure Terror! The House of Orochimaru',
		type: 'Filler',
		explanation:
			"This filler episode deals with Orochimaru's hideouts but does not add anything to his character or the overall plot."
	},
	{
		number: 140,
		title: "Two Heartbeats: Kabuto's Trap",
		type: 'Filler',
		explanation:
			'Part of a filler arc that continues to explore characters and events not present in the manga, making it non-essential.'
	},
	{
		number: 141,
		title: "Sakura's Determination",
		type: 'Mixed Canon/Filler',
		explanation: 'Continuation of the filler arc.'
	},
	{
		number: 142,
		title: 'The Three Villains from the Maximum Security Prison',
		type: 'Filler',
		explanation:
			'New filler arc about a mission involving three escaped prisoners. Plot unrelated to the main story.'
	},
	{
		number: 143,
		title: "Ton Ton! I'm Counting on You!",
		type: 'Filler',
		explanation:
			'A filler episode featuring a mission led by Ton Ton, Tsunade’s pet pig. It is unrelated to any ongoing plotlines.'
	},
	{
		number: 144,
		title: 'A New Squad! Two People and a Dog?!',
		type: 'Filler',
		explanation:
			'Part of a filler arc, focusing on a mission led by Naruto, Hinata, and Kiba. This storyline does not contribute to the main narrative.'
	},
	{
		number: 145,
		title: 'A New Formation: Ino-Shika-Cho!',
		type: 'Filler',
		explanation:
			'Continues the filler arc. Although it involves popular characters, the story arc is not part of the main plot.'
	},
	{
		number: 146,
		title: "Orochimaru's Shadow",
		type: 'Filler',
		explanation:
			"Another episode in a filler storyline about Orochimaru's schemes, but it does not reveal anything new or important."
	},
	{
		number: 147,
		title: "A Clash of Fate: You Can't Bring Me Down",
		type: 'Filler',
		explanation:
			'Filler episode that continues an unimportant subplot. Skippable without losing any important story developments.'
	},
	{
		number: 148,
		title: 'The Search for the Rare Bikochu Beetle',
		type: 'Filler',
		explanation:
			'Introduces a new mission involving the search for a rare beetle. Recommended Watch.'
	},
	{
		number: 149,
		title: "What's the Difference? Don't All Insects Look Alike?",
		type: 'Filler',
		explanation:
			'Part of the Bikochu beetle filler arc. This episode does not contain any key plot developments.'
	},
	{
		number: 150,
		title: 'A Battle of Bugs! The Deceivers and the Deceived',
		type: 'Filler',
		explanation:
			'Continuation of the filler arc about the Bikochu beetle. It adds no new information to the main storyline.'
	},
	{
		number: 151,
		title: 'Blaze Away, Byakugan! This is My Ninja Way!',
		type: 'Filler',
		explanation:
			'Concludes the Bikochu beetle filler arc. This arc is not referenced in the main story.'
	},
	{
		number: 152,
		title: 'Funeral March for the Living',
		type: 'Filler',
		explanation:
			'Begins a new filler arc focusing on a mission with minor characters. The story is not connected to the primary plotline.'
	},
	{
		number: 153,
		title: 'A Lesson Learned: The Iron Fist of Love',
		type: 'Filler',
		explanation:
			'Continues the Seven Ninja Swordsmen filler arc. Does not tie into any major story arcs or characters.'
	},
	{
		number: 154,
		title: 'The Enemy of the Byakugan',
		type: 'Filler',
		explanation:
			'Part of a filler arc focusing on Hinata and the Byakugan. It has no bearing on the main plot.'
	},
	{
		number: 155,
		title: 'The Dark Creeping Clouds',
		type: 'Filler',
		explanation:
			'Filler episode continuing an unrelated mission. It does not contribute to the main storyline.'
	},
	{
		number: 156,
		title: "Raiga's Counterattack",
		type: 'Filler',
		explanation:
			'Continuation of a filler arc featuring a character named Raiga. This storyline is not connected to the main narrative.'
	},
	{
		number: 157,
		title: 'Run! The Curry of Life',
		type: 'Filler',
		explanation:
			"A lighthearted filler episode focusing on the 'Curry of Life.' It does not affect any core plot points."
	},
	{
		number: 158,
		title: 'Follow My Lead! The Great Survival Challenge',
		type: 'Filler',
		explanation:
			'A standalone filler episode featuring a training mission with new students. It focuses on teamwork and survival skills, but does not contribute to the main plot.'
	},
	{
		number: 159,
		title: 'The Bounty Hunter from the Wilderness',
		type: 'Filler',
		explanation:
			'This filler episode introduces a character named Gansuki, a bounty hunter. The plot is self-contained and does not connect to the main storyline.'
	},
	{
		number: 160,
		title: 'Hunt or Be Hunted?! Showdown at the O.K. Temple',
		type: 'Filler',
		explanation:
			'Continuation of the filler arc involving Gansuki. The story does not add anything significant to the overall plot of the series.'
	},
	{
		number: 161,
		title: 'The Appearance of Strange Visitors',
		type: 'Filler',
		explanation:
			'A skippable filler episode featuring a humorous, standalone story with characters that do not reappear in the main plot.'
	},
	{
		number: 162,
		title: 'The Cursed Warrior',
		type: 'Filler',
		explanation:
			'Part of a filler arc involving a ghostly warrior. The storyline is not relevant to the primary events of Naruto and can be skipped.'
	},
	{
		number: 163,
		title: "The Tactician's Intent",
		type: 'Filler',
		explanation:
			"Continuation of the 'Cursed Warrior' arc. It contains a self-contained mystery that does not affect the main story."
	},
	{
		number: 164,
		title: 'Too Late for Help',
		type: 'Filler',
		explanation:
			"Part of the 'Cursed Warrior' filler arc, focusing on a subplot that has no bearing on the overarching storyline."
	},
	{
		number: 165,
		title: 'The Death of Naruto',
		type: 'Filler',
		explanation:
			'Despite its dramatic title, this filler episode is not related to the core plot and involves a false premise. It can be skipped.'
	},
	{
		number: 166,
		title: 'When Time Stands Still',
		type: 'Filler',
		explanation:
			"Continuation of the 'Cursed Warrior' arc. This filler episode does not introduce any new canon elements."
	},
	{
		number: 167,
		title: 'When Egrets Flap Their Wings',
		type: 'Filler',
		explanation:
			"The conclusion of the 'Cursed Warrior' filler arc. It wraps up the story without adding to the main narrative."
	},
	{
		number: 168,
		title: 'Mix It, Stretch It, Boil It Up! Burn Copper Pot, Burn!',
		type: 'Filler',
		explanation: 'Comedic filler episode featuring Naruto and the ramen chef. Recommended Watch.'
	},
	{
		number: 169,
		title: 'Remembrance: The Lost Page',
		type: 'Filler',
		explanation:
			'This filler episode involves a mission to recover a lost page. It does not connect to the main storyline and is skippable.'
	},
	{
		number: 170,
		title: 'The Closed Door',
		type: 'Filler',
		explanation:
			'A filler episode that continues the story of a mission unrelated to the primary plot. It introduces no new significant details.'
	},
	{
		number: 171,
		title: 'Infiltration: The Set-Up!',
		type: 'Filler',
		explanation:
			'Part of a filler arc that follows a mission to infiltrate an enemy base. The plot does not tie into the main events of the series.'
	},
	{
		number: 172,
		title: 'Despair: A Fractured Heart',
		type: 'Filler',
		explanation:
			'Continues the filler arc. Focuses on character interactions and conflicts that are not part of the central storyline.'
	},
	{
		number: 173,
		title: 'The Battle at Sea: The Power Unleashed!',
		type: 'Filler',
		explanation:
			'A filler episode involving a sea battle. It does not contribute to the main plot of Naruto.'
	},
	{
		number: 174,
		title: 'Impossible! Celebrity Ninja Art - Money Style Jutsu',
		type: 'Filler',
		explanation:
			'A comedic filler episode featuring a parody of celebrity culture. It is unrelated to the main narrative.'
	},
	{
		number: 175,
		title: 'The Treasure Hunt is On!',
		type: 'Filler',
		explanation:
			'A filler episode centered around a treasure hunt. The plot is lighthearted and does not affect the core storyline.'
	},
	{
		number: 176,
		title: 'Run, Dodge, Zigzag! Chase or Be Chased!',
		type: 'Filler',
		explanation:
			'A skippable filler episode that focuses on a humorous chase. It has no bearing on the main events of the series.'
	},
	{
		number: 177,
		title: 'Please, Mr. Postman!',
		type: 'Filler',
		explanation:
			'Another standalone filler episode. It features a simple plot involving mail delivery, with no connection to the main story.'
	},
	{
		number: 178,
		title: "Encounter! The Boy with a Star's Name",
		type: 'Filler',
		explanation:
			'Part of a filler arc set in a village with a unique star training. This story is not part of the original manga plot.'
	},
	{
		number: 179,
		title: 'The Remembered Lullaby',
		type: 'Filler',
		explanation:
			"Continues the 'Star Village' arc. The storyline is exclusive to the anime and does not affect the main plot."
	},
	{
		number: 180,
		title: 'Hidden Jutsu! The Price of The Ninja Art: Kujaku',
		type: 'Filler',
		explanation:
			"Part of the 'Star Village' filler arc. It focuses on a technique unique to the village, which does not appear in the main storyline."
	},
	{
		number: 181,
		title: 'Hoshikage: The Buried Truth',
		type: 'Filler',
		explanation:
			"This filler episode concludes the 'Star Village' arc. It has no impact on the main plot."
	},
	{
		number: 182,
		title: 'Reunion: The Remaining Time',
		type: 'Filler',
		explanation:
			'Another filler episode, focusing on minor character stories. It is not part of the central narrative.'
	},
	{
		number: 183,
		title: "The Star's Radiance",
		type: 'Filler',
		explanation:
			"Wraps up the 'Star Village' arc. This arc can be skipped without losing any important plot details."
	},
	{
		number: 184,
		title: "Kiba's Long Day",
		type: 'Filler',
		explanation: 'A lighthearted filler featuring Kiba and Akamaru. Skippable.'
	},
	{
		number: 185,
		title: 'A Legend from the Hidden Leaf: The Onbaa',
		type: 'Filler',
		explanation:
			'Comedic filler episode about a creature called Onbaa. Does not affect the main plot.'
	},
	{
		number: 186,
		title: 'Laughing Shino',
		type: 'Filler',
		explanation: 'Humorous filler episode centered on Shino behaving unusually. Recommended Watch.'
	},
	{
		number: 187,
		title: 'Open for Business! The Leaf Moving Service',
		type: 'Filler',
		explanation:
			'Standalone filler episode involving a moving service run by the characters. Not related to the main story.'
	},
	{
		number: 188,
		title: 'Mystery of the Targeted Merchants',
		type: 'Filler',
		explanation:
			'This episode is part of a filler arc with a mystery plot. Not connected to the primary storyline.'
	},
	{
		number: 189,
		title: 'A Limitless Supply of Ninja Tools',
		type: 'Filler',
		explanation: 'Filler episode that explores a mission involving ninja tools. Skippable.'
	},
	{
		number: 190,
		title: 'The Byakugan Sees the Blind Spot!',
		type: 'Filler',
		explanation: "Standalone episode that focuses on Hinata's Byakugan ability and training."
	},
	{
		number: 191,
		title: 'Forecast: Death! Cloudy with Chance of Sun!',
		type: 'Filler',
		explanation: 'Filler episode with a mystery involving strange weather patterns. Skippable.'
	},
	{
		number: 192,
		title: 'Ino Screams! Chubby Paradise!',
		type: 'Filler',
		explanation: 'Humorous filler episode focused on a mission involving Ino. Recommended Watch.'
	},
	{
		number: 193,
		title: 'Viva Dojo Challenge! Youth Is All About Passion!',
		type: 'Filler',
		explanation: 'A comedic filler featuring Lee and Gai in a dojo challenge. Recommended Watch.'
	},
	{
		number: 194,
		title: 'The Mysterious Curse of the Haunted Castle',
		type: 'Filler',
		explanation: 'A spooky, standalone filler episode set in a haunted castle. Skippable.'
	},
	{
		number: 195,
		title: 'The Third Super-Beast!',
		type: 'Filler',
		explanation:
			'Filler episode focusing on the rivalry between Lee and Gai. It introduces a minor character but has no relevance to the main plot.'
	},
	{
		number: 196,
		title: 'Hot-Blooded Confrontation: Student vs. Sensei',
		type: 'Filler',
		explanation:
			'Lighthearted filler episode featuring sparring matches between students and their mentors.'
	},
	{
		number: 197,
		title: 'Crisis: The Hidden Leaf 11 Gather!',
		type: 'Filler',
		explanation:
			'Filler episode that brings together several characters for a minor mission. Skippable.'
	},
	{
		number: 198,
		title: "The ANBU Gives Up? Naruto's Recollection",
		type: 'Filler',
		explanation: "Recap-style filler episode focusing on past events from Naruto's perspective."
	},
	{
		number: 199,
		title: 'The Missed Target',
		type: 'Filler',
		explanation:
			'Standalone filler episode involving a mission that is unrelated to the main story.'
	},
	{
		number: 200,
		title: 'The Powerful Helper',
		type: 'Filler',
		explanation:
			'Filler episode around a mission involving a new character. The plot is not connected to the main storyline.'
	},
	{
		number: 201,
		title: 'Multiple Traps! Countdown to Destruction',
		type: 'Filler',
		explanation:
			'Part of a filler arc involving a mission with a countdown element. It does not influence the overall plot of the series.'
	},
	{
		number: 202,
		title: 'The Top 5 Ninja Battles',
		type: 'Filler',
		explanation:
			'Recap-style filler episode that reviews notable battles in the series. It offers no new story content and can be skipped.'
	},
	{
		number: 203,
		title: "Kurenai's Decision: Squad 8 Left Behind",
		type: 'Filler',
		explanation:
			'Filler episode focusing on Kurenai and her team. It explores their dynamics but does not add to the main plot.'
	},
	{
		number: 204,
		title: "Yakumo's Sealed Power",
		type: 'Filler',
		explanation:
			'Part of a filler arc about a character named Yakumo. This arc is exclusive to the anime.'
	},
	{
		number: 205,
		title: "Kurenai's Top-Secret Mission: The Promise with the Third Hokage",
		type: 'Filler',
		explanation:
			"Continuation of the Yakumo arc. It delves into backstory elements that are not relevant to the manga's plot."
	},
	{
		number: 206,
		title: 'Genjutsu or Reality?',
		type: 'Filler',
		explanation:
			'Part of the Yakumo filler arc. It features a focus on genjutsu but does not contribute to the core storyline.'
	},
	{
		number: 207,
		title: 'The Supposed Sealed Ability',
		type: 'Filler',
		explanation: 'Continuation of the Yakumo arc. The plot remains filler and skippable.'
	},
	{
		number: 208,
		title: 'The Weight of the Prized Artifact',
		type: 'Filler',
		explanation:
			'Episode revolving around the protection of an artifact. It is a standalone story that does not connect to the main narrative.'
	},
	{
		number: 209,
		title: 'The Enemy: Ninja Dropouts',
		type: 'Filler',
		explanation: 'Part of a filler arc featuring rogue ninja. Skippable.'
	},
	{
		number: 210,
		title: 'The Bewildering Forest',
		type: 'Filler',
		explanation: 'Continuation of the rogue ninja arc. Skippable.'
	},
	{
		number: 211,
		title: 'Memory of Flames',
		type: 'Filler',
		explanation:
			'Another episode in the rogue ninja arc. It introduces minor characters and conflicts not found in the main story.'
	},
	{
		number: 212,
		title: 'To Each His Own Path',
		type: 'Filler',
		explanation:
			'A filler episode focusing on personal conflicts within the rogue ninja arc. Skippable.'
	},
	{
		number: 213,
		title: 'Vanished Memories',
		type: 'Filler',
		explanation: 'Continues the filler arc without adding new canon elements. Skippable.'
	},
	{
		number: 214,
		title: 'Bringing Back Reality',
		type: 'Filler',
		explanation:
			'Part of the rogue ninja filler arc, involving a mission that does not affect the overall plot.'
	},
	{
		number: 215,
		title: 'A Past to Be Erased',
		type: 'Filler',
		explanation:
			'Concludes the rogue ninja filler arc. The story arc does not influence the main events of Naruto.'
	},
	{
		number: 216,
		title: 'The Targeted Shukaku',
		type: 'Filler',
		explanation:
			'Starts a new filler arc involving Gaara and Shukaku. The plot is not part of the manga.'
	},
	{
		number: 217,
		title: 'Sand Alliance with the Leaf Shinobi',
		type: 'Filler',
		explanation:
			'Continuation of the filler arc involving a mission to protect the Sand Village, Can be skipped.'
	},
	{
		number: 218,
		title: 'The Counterattack!',
		type: 'Filler',
		explanation:
			'Part of the Sand Village filler arc. Features battles that are unrelated to the main plot.'
	},
	{
		number: 219,
		title: 'The Ultimate Weapon Reborn',
		type: 'Filler',
		explanation:
			'Concludes the Sand Village filler arc. The events are not canon and do not affect the core storyline.'
	},
	{
		number: 220,
		title: 'Departure',
		type: 'Mixed Canon/Filler',
		explanation:
			'Some canon elements towards the end. If you skip the filler arc (216-219), watch starting from 12:18.'
	}
];

export const narutoArcs: Arc[] = [
	{
		name: 'Prologue - Land of Waves',
		episode_start: 1,
		episode_end: 19
	},
	{
		name: 'Chunin Exams - The Written Exam & Forest of Death',
		episode_start: 20,
		episode_end: 44
	},
	{
		name: 'Chunin Exams - The Preliminary Rounds',
		episode_start: 45,
		episode_end: 51
	},
	{
		name: 'Chunin Exams - The Final Rounds & Konoha Crush',
		episode_start: 52,
		episode_end: 80
	},
	{
		name: 'Search for Tsunade',
		episode_start: 81,
		episode_end: 100
	},
	{
		name: 'Sasuke Recovery Mission',
		episode_start: 107,
		episode_end: 135
	},
	{
		name: 'Land of Tea Escort Mission',
		episode_start: 102,
		episode_end: 106
	},
	{
		name: 'Land of Rice Fields Investigation Mission',
		episode_start: 136,
		episode_end: 141
	},
	{
		name: 'Mizuki Tracking Mission',
		episode_start: 142,
		episode_end: 147
	},
	{
		name: 'Bikochu Search Mission',
		episode_start: 148,
		episode_end: 151
	},
	{
		name: 'Kurosuki Family Removal Mission',
		episode_start: 152,
		episode_end: 157
	},
	{
		name: 'Buried Gold Excavation Mission',
		episode_start: 158,
		episode_end: 160
	},
	{
		name: 'Peddlers Escort Mission',
		episode_start: 161,
		episode_end: 167
	},
	{
		name: 'Star Guard Mission',
		episode_start: 178,
		episode_end: 183
	},
	{
		name: 'Cursed Warrior Extermination Mission',
		episode_start: 162,
		episode_end: 167
	},
	{
		name: 'Sea Country Mission',
		episode_start: 169,
		episode_end: 173
	},
	{
		name: 'Konoha Plans Recapture Mission',
		episode_start: 174,
		episode_end: 176
	},
	{
		name: 'Gantetsu Escort Mission',
		episode_start: 209,
		episode_end: 212
	},
	{
		name: 'Menma Memory Search Mission',
		episode_start: 213,
		episode_end: 215
	},
	{
		name: 'Sunagakure Support Mission',
		episode_start: 216,
		episode_end: 220
	}
];

export const narutoQuickList: QuickList = {
	Manga_Canon_Episodes: [
		'1-6',
		'8',
		'10-13',
		'17',
		'22',
		'25',
		'31-36',
		'42',
		'48',
		'50-51',
		'61-62',
		'64-65',
		'67-68',
		'73',
		'75-82',
		'84-96',
		'107-111',
		'115-125',
		'128-129',
		'132-135'
	],
	Mixed_Canon_Filler_Episodes: [
		'7',
		'9',
		'14-16',
		'18-21',
		'23-24',
		'27-30',
		'37-41',
		'43-47',
		'49',
		'52-60',
		'63',
		'66',
		'69-72',
		'74',
		'83',
		'98-100',
		'112-114',
		'126-127',
		'130-131',
		'141-142',
		'220'
	],
	Filler_Episodes: ['26', '97', '101-106', '136-140', '143-219']
};

export const narutoShippuden: Episode[] = [
	{
		number: 1,
		title: 'Homecoming',
		type: 'Mixed Canon/Filler'
	},
	{
		number: 2,
		title: 'The Akatsuki Makes Its Move',
		type: 'Mixed Canon/Filler'
	},
	{
		number: 3,
		title: 'The Results of Training',
		type: 'Mixed Canon/Filler'
	},
	{
		number: 4,
		title: 'The Jinchuriki of the Sand',
		type: 'Mixed Canon/Filler'
	},
	{
		number: 5,
		title: 'The Kazekage Stands Tall',
		type: 'Mixed Canon/Filler'
	},
	{
		number: 6,
		title: 'Mission Cleared',
		type: 'Mixed Canon/Filler'
	},
	{
		number: 7,
		title: 'Run, Kankuro',
		type: 'Mixed Canon/Filler'
	},
	{
		number: 8,
		title: 'Team Kakashi, Deployed',
		type: 'Mixed Canon/Filler'
	},
	{
		number: 9,
		title: "The Jinchuriki's Tears",
		type: 'Mixed Canon/Filler'
	},
	{
		number: 10,
		title: 'Sealing Jutsu: Nine Phantom Dragons',
		type: 'Mixed Canon/Filler'
	},
	{
		number: 11,
		title: "The Medical Ninja's Student",
		type: 'Mixed Canon/Filler'
	},
	{
		number: 12,
		title: "The Retired Granny's Determination",
		type: 'Mixed Canon/Filler'
	},
	{
		number: 13,
		title: 'A Meeting With Destiny',
		type: 'Mixed Canon/Filler'
	},
	{
		number: 14,
		title: "Naruto's Growth",
		type: 'Mixed Canon/Filler'
	},
	{
		number: 15,
		title: 'The Secret Weapon is Called....',
		type: 'Mixed Canon/Filler'
	},
	{
		number: 16,
		title: 'The Secret of Jinchuriki',
		type: 'Mixed Canon/Filler'
	},
	{
		number: 17,
		title: 'The Death of Gaara!',
		type: 'Mixed Canon/Filler'
	},
	{
		number: 18,
		title: 'Charge Tactic! Button Hook Entry!!',
		type: 'Mixed Canon/Filler'
	},
	{
		number: 19,
		title: "Traps Activate! Team Guy's Enemies",
		type: 'Mixed Canon/Filler'
	},
	{
		number: 20,
		title: 'Hiruko vs. Two Kunoichi!',
		type: 'Canon'
	},
	{
		number: 21,
		title: "Sasori's Real Face",
		type: 'Canon'
	},
	{
		number: 22,
		title: "Chiyo's Secret Skills",
		type: 'Canon'
	},
	{
		number: 23,
		title: 'Father and Mother',
		type: 'Canon'
	},
	{
		number: 24,
		title: 'The Third Kazekage',
		type: 'Canon'
	},
	{
		number: 25,
		title: 'Three Minutes Between Life and Death',
		type: 'Canon'
	},
	{
		number: 26,
		title: 'Puppet Fight: 10 vs. 100!',
		type: 'Canon'
	},
	{
		number: 27,
		title: 'Impossible Dream',
		type: 'Canon'
	},
	{
		number: 28,
		title: 'Beasts: Alive Again!',
		type: 'Filler',
		explanation:
			'A filler episode featuring Team Guy facing clones of themselves. Skippable as it adds no major plot points.'
	},
	{
		number: 29,
		title: 'Kakashi Enlightened!',
		type: 'Canon'
	},
	{
		number: 30,
		title: 'Aesthetics of an Instant',
		type: 'Canon'
	},
	{
		number: 31,
		title: 'The Legacy',
		type: 'Canon'
	},
	{
		number: 32,
		title: 'Return of the Kazekage',
		type: 'Canon'
	},
	{
		number: 33,
		title: 'The New Target',
		type: 'Canon'
	},
	{
		number: 34,
		title: 'Formation! New Team Kakashi',
		type: 'Canon'
	},
	{
		number: 35,
		title: 'An Unnecessary Addition',
		type: 'Canon'
	},
	{
		number: 36,
		title: 'The Fake Smile',
		type: 'Canon'
	},
	{
		number: 37,
		title: 'Untitled',
		type: 'Canon'
	},
	{
		number: 38,
		title: 'Simulation',
		type: 'Canon'
	},
	{
		number: 39,
		title: 'The Tenchi Bridge',
		type: 'Canon'
	},
	{
		number: 40,
		title: 'Nine-Tails Unleashed',
		type: 'Canon'
	},
	{
		number: 41,
		title: 'The Top-Secret Mission Begins',
		type: 'Canon'
	},
	{
		number: 42,
		title: 'Orochimaru vs. Jinchuriki',
		type: 'Canon'
	},
	{
		number: 43,
		title: "Sakura's Tears",
		type: 'Canon'
	},
	{
		number: 44,
		title: 'The Secret of the Battle!',
		type: 'Canon'
	},
	{
		number: 45,
		title: 'The Consequences of Betrayal',
		type: 'Canon'
	},
	{
		number: 46,
		title: 'The Unfinished Page',
		type: 'Canon'
	},
	{
		number: 47,
		title: 'Infiltration: The Den of the Snake!',
		type: 'Canon'
	},
	{
		number: 48,
		title: 'Bonds',
		type: 'Canon'
	},
	{
		number: 49,
		title: 'Something Important...',
		type: 'Canon'
	},
	{
		number: 50,
		title: "The Picture Book's Story",
		type: 'Canon'
	},
	{
		number: 51,
		title: 'Reunion',
		type: 'Canon'
	},
	{
		number: 52,
		title: 'The Power of Uchiha',
		type: 'Canon'
	},
	{
		number: 53,
		title: 'Title',
		type: 'Canon'
	},
	{
		number: 54,
		title: 'Nightmare',
		type: 'Canon'
	},
	{
		number: 55,
		title: 'Wind',
		type: 'Canon'
	},
	{
		number: 56,
		title: 'Squirming',
		type: 'Canon'
	},
	{
		number: 57,
		title: 'Robbed of Sleep',
		type: 'Filler',
		explanation:
			'Filler episode revolving around the Fire Temple. Skippable as it doesn’t add much to the story.'
	},
	{
		number: 58,
		title: 'Loneliness',
		type: 'Filler',
		explanation:
			'Part of the Fire Temple filler arc. Focuses on the temple monks but is not essential to the main plot.'
	},
	{
		number: 59,
		title: 'A New Enemy',
		type: 'Filler',
		explanation:
			'Introduces a new enemy within the filler arc. Skippable unless you are watching the arc.'
	},
	{
		number: 60,
		title: 'Impermanence',
		type: 'Filler',
		explanation:
			'Continues the filler arc, exploring the lives of the Fire Temple monks. Skippable.'
	},
	{
		number: 61,
		title: 'Contact',
		type: 'Filler',
		explanation: 'Another filler episode with little relevance to the main plot. Can be skipped.'
	},
	{
		number: 62,
		title: 'Teammate',
		type: 'Filler',
		explanation:
			'Explores minor character interactions. It’s skippable unless you’re following the filler arc.'
	},
	{
		number: 63,
		title: 'The Two Kings',
		type: 'Filler',
		explanation:
			'Wraps up the Fire Temple arc, but it doesn’t contribute significantly to the main story. Skippable.'
	},
	{
		number: 64,
		title: 'The Jet-Black Signal Fire',
		type: 'Filler',
		explanation: 'This is a filler episode leading to more filler events. Skippable.'
	},
	{
		number: 65,
		title: 'Lockdown of Darkness',
		type: 'Filler',
		explanation: 'Filler episode with no major plot relevance. Skippable.'
	},
	{
		number: 66,
		title: 'Revived Souls',
		type: 'Filler',
		explanation: 'Continues the filler arc but has little impact on the main story. Skippable.'
	},
	{
		number: 67,
		title: "Everyone's Struggle to the Death",
		type: 'Filler',
		explanation: 'Filler episode with minor relevance to the ongoing arc. Skippable.'
	},
	{
		number: 68,
		title: 'Moment of Awakening',
		type: 'Filler',
		explanation: 'Part of the filler arc. It can be skipped unless following the arc.'
	},
	{
		number: 69,
		title: 'Despair',
		type: 'Filler',
		explanation: 'The filler arc continues but can be skipped if focusing on canon.'
	},
	{
		number: 70,
		title: 'Resonance',
		type: 'Filler',
		explanation: 'Another filler episode in the same arc. It’s skippable unless following the arc.'
	},
	{
		number: 71,
		title: 'My Friend',
		type: 'Filler',
		explanation: 'Filler episode that concludes this minor arc. Skippable.'
	},
	{
		number: 72,
		title: 'The Quietly Approaching Threat',
		type: 'Canon'
	},
	{
		number: 73,
		title: "Akatsuki's Invasion",
		type: 'Canon'
	},
	{
		number: 74,
		title: 'Under the Starry Sky',
		type: 'Canon'
	},
	{
		number: 75,
		title: "The Old Monk's Prayer",
		type: 'Canon'
	},
	{
		number: 76,
		title: 'The Next Step',
		type: 'Canon'
	},
	{
		number: 77,
		title: 'Climbing Silver',
		type: 'Canon'
	},
	{
		number: 78,
		title: 'The Judgment',
		type: 'Canon'
	},
	{
		number: 79,
		title: 'Unfulfilled Scream',
		type: 'Canon'
	},
	{
		number: 80,
		title: 'Last Words',
		type: 'Canon'
	},
	{
		number: 81,
		title: 'Sad News',
		type: 'Canon'
	},
	{
		number: 82,
		title: 'Team Ten',
		type: 'Canon'
	},
	{
		number: 83,
		title: 'Target: Locked On',
		type: 'Canon'
	},
	{
		number: 84,
		title: "Kakuzu's Abilities",
		type: 'Canon'
	},
	{
		number: 85,
		title: 'The Terrifying Secret',
		type: 'Canon'
	},
	{
		number: 86,
		title: "Shikamaru's Genius",
		type: 'Canon'
	},
	{
		number: 87,
		title: 'When You Curse Someone, You Dig Your Own Grave',
		type: 'Canon'
	},
	{
		number: 88,
		title: 'Wind Style: Rasen Shuriken!',
		type: 'Canon'
	},
	{
		number: 89,
		title: 'The Price of Power',
		type: 'Mixed Canon/Filler',
		explanation:
			"Begins the Three-Tails' Appearance arc, blending canon storylines with filler scenes that set up the arc's conflict."
	},
	{
		number: 90,
		title: "A Shinobi's Determination",
		type: 'Mixed Canon/Filler',
		explanation:
			"Continues the Three-Tails' Appearance arc, with a mix of canon moments and filler content to build character motivation."
	},

	{
		number: 91,
		title: "Orochimaru's Hideout Discovered",
		type: 'Filler',
		explanation:
			"The start of the Three-Tails arc. It involves Team Kakashi's investigation of Orochimaru’s hideout, which is unrelated to the main storyline."
	},
	{
		number: 92,
		title: 'Encounter',
		type: 'Filler',
		explanation:
			"Team Kakashi meets new enemies in this filler arc while following Orochimaru's trail, with no impact on the main plot."
	},
	{
		number: 93,
		title: 'Connecting Hearts',
		type: 'Filler',
		explanation:
			'Focuses on developing relationships between characters during the Three-Tails arc, but is unrelated to the main story.'
	},
	{
		number: 94,
		title: 'A Night of Rain',
		type: 'Filler',
		explanation:
			'A filler episode continuing the Three-Tails arc, featuring character interactions that don’t influence the main plot.'
	},
	{
		number: 95,
		title: 'The Two Charms',
		type: 'Filler',
		explanation:
			"Expands on the Three-Tails arc with some light emotional moments between characters, but doesn't impact the core narrative."
	},
	{
		number: 96,
		title: 'The Unseeing Enemy',
		type: 'Filler',
		explanation:
			'Filler episode in the Three-Tails arc where Team Kakashi faces off against new enemies with no significant relation to the main plot.'
	},
	{
		number: 97,
		title: 'The Labyrinth of Distorted Reflection',
		type: 'Filler',
		explanation:
			'A filler episode involving a new threat in the Three-Tails arc, unrelated to the central storyline.'
	},
	{
		number: 98,
		title: 'The Target Appears',
		type: 'Filler',
		explanation:
			'More of the Three-Tails arc unfolds with filler content about a new target, but no connection to the main plot.'
	},
	{
		number: 99,
		title: 'The Rampaging Tailed Beast',
		type: 'Filler',
		explanation:
			'A filler episode in the Three-Tails arc where the tailed beast becomes active, unrelated to the main story.'
	},
	{
		number: 100,
		title: 'Inside the Mist',
		type: 'Filler',
		explanation:
			'Continues the Three-Tails arc, where Team Kakashi navigates a misty environment in another filler episode.'
	},
	{
		number: 101,
		title: "Everyone's Feelings",
		type: 'Filler',
		explanation:
			'This filler episode focuses on character emotions and bonds in the Three-Tails arc, but is not connected to the main plot.'
	},
	{
		number: 102,
		title: 'Regroup!',
		type: 'Filler',
		explanation:
			'Part of the Three-Tails arc, this filler episode features the team regrouping after battles, with no impact on the core story.'
	},
	{
		number: 103,
		title: 'The Four-Corner Sealing Barrier',
		type: 'Filler',
		explanation:
			'Filler episode focusing on a sealing jutsu during the Three-Tails arc, unrelated to the main plot.'
	},
	{
		number: 104,
		title: 'Breaking the Crystal Style',
		type: 'Filler',
		explanation:
			'Team Kakashi attempts to break a crystal jutsu in this filler episode, which has no connection to the main storyline.'
	},
	{
		number: 105,
		title: 'The Battle Over the Barrier',
		type: 'Filler',
		explanation:
			'A filler episode in the Three-Tails arc focusing on battles over the barrier, not related to the core story.'
	},
	{
		number: 106,
		title: 'Red Camellia',
		type: 'Filler',
		explanation:
			'A filler episode where secondary characters get more development, with no impact on the overall story.'
	},
	{
		number: 107,
		title: 'Strange Bedfellows',
		type: 'Filler',
		explanation:
			'Another filler episode from the Three-Tails arc, focusing on shifting alliances with no effect on the main plot.'
	},
	{
		number: 108,
		title: 'Guidepost of the Camellia',
		type: 'Filler',
		explanation:
			'Filler episode that builds on character relationships in the Three-Tails arc without impacting the central narrative.'
	},
	{
		number: 109,
		title: 'Cursed Seal Counterattack',
		type: 'Filler',
		explanation:
			'Focuses on a battle involving the Cursed Seal during the filler-heavy Three-Tails arc.'
	},
	{
		number: 110,
		title: 'Memory of Guilt',
		type: 'Filler',
		explanation:
			'The Three-Tails arc continues in this filler episode, starting to wind down towards the arc’s conclusion.'
	},
	{
		number: 111,
		title: 'Shattered Promise',
		type: 'Filler',
		explanation:
			'A filler episode that begins wrapping up the Three-Tails arc, with some minor connections to later canon.'
	},
	{
		number: 112,
		title: 'A Place to Return To',
		type: 'Filler',
		explanation:
			'Concludes the filler-heavy Three-Tails arc, leading back to canon episodes shortly afterward.'
	},
	{
		number: 113,
		title: "The Serpent's Pupil",
		type: 'Canon'
	},
	{
		number: 114,
		title: 'Eye of the Hawk',
		type: 'Canon'
	},
	{
		number: 115,
		title: "Zabuza's Blade",
		type: 'Canon'
	},
	{
		number: 116,
		title: 'Guardian of the Iron Wall',
		type: 'Canon'
	},
	{
		number: 117,
		title: 'Jugo of the Northern Hideout',
		type: 'Canon'
	},
	{
		number: 118,
		title: 'Formation!',
		type: 'Canon'
	},
	{
		number: 119,
		title: "Kakashi Chronicles - A Boy's Life on the Battlefield - Part 1",
		type: 'Canon'
	},
	{
		number: 120,
		title: "Kakashi Chronicles - A Boy's Life on the Battlefield - Part 2",
		type: 'Canon'
	},
	{
		number: 121,
		title: 'Assemble',
		type: 'Canon'
	},
	{
		number: 122,
		title: 'The Hunt',
		type: 'Canon'
	},
	{
		number: 123,
		title: 'Clash!',
		type: 'Canon'
	},
	{
		number: 124,
		title: 'Art',
		type: 'Canon'
	},
	{
		number: 125,
		title: 'Disappearance',
		type: 'Canon'
	},
	{
		number: 126,
		title: 'Twilight',
		type: 'Canon'
	},
	{
		number: 127,
		title: 'Tales of a Gutsy Ninja ~Jiraiya Ninja Scroll~ Part 1',
		type: 'Canon'
	},
	{
		number: 128,
		title: 'Tales of a Gutsy Ninja ~Jiraiya Ninja Scroll~ Part 2',
		type: 'Canon'
	},
	{
		number: 129,
		title: 'Infiltrate! The Village Hidden in the Rain',
		type: 'Canon'
	},
	{
		number: 130,
		title: 'The Man Who Became God',
		type: 'Canon'
	},
	{
		number: 131,
		title: 'Honored Sage Mode!',
		type: 'Canon'
	},
	{
		number: 132,
		title: 'In Attendance, the Six Paths of Pain',
		type: 'Canon'
	},
	{
		number: 133,
		title: 'The Tale of Jiraiya the Gallant',
		type: 'Canon'
	},
	{
		number: 134,
		title: 'Banquet Invitation',
		type: 'Canon'
	},
	{
		number: 135,
		title: 'The Longest Moment',
		type: 'Canon'
	},
	{
		number: 136,
		title: 'The Light & Dark of the Mangekyo Sharingan',
		type: 'Canon'
	},
	{
		number: 137,
		title: 'Amaterasu!',
		type: 'Canon'
	},
	{
		number: 138,
		title: 'The End',
		type: 'Canon'
	},
	{
		number: 139,
		title: 'The Mystery of Tobi',
		type: 'Canon'
	},
	{
		number: 140,
		title: 'Fate',
		type: 'Canon'
	},
	{
		number: 141,
		title: 'Truth',
		type: 'Canon'
	},
	{
		number: 142,
		title: 'Battle of Unraikyo',
		type: 'Canon'
	},
	{
		number: 143,
		title: 'The Eight-Tails vs. Sasuke',
		type: 'Canon'
	},
	{
		number: 144,
		title: 'Wanderer',
		type: 'Filler',
		explanation:
			'Part of the Six-Tails Unleashed arc. This filler introduces some characters connected to canon. Recommended to watch.'
	},
	{
		number: 145,
		title: 'Successor of the Forbidden Jutsu',
		type: 'Filler',
		explanation:
			'Continuation of the Six-Tails Unleashed arc. Worth watching for canon connections.'
	},
	{
		number: 146,
		title: "The Successor's Wish",
		type: 'Filler',
		explanation:
			'Another episode in the Six-Tails Unleashed arc. Recommended to watch for additional background.'
	},
	{
		number: 147,
		title: "Rogue Ninja's Past",
		type: 'Filler',
		explanation:
			'Important for the Six-Tails Unleashed arc and offers backstory relevant to the main story.'
	},
	{
		number: 148,
		title: 'Heir to Darkness',
		type: 'Filler',
		explanation:
			'Continuation of the Six-Tails Unleashed arc. Recommended for its connections to upcoming canon.'
	},
	{
		number: 149,
		title: 'Parting',
		type: 'Filler',
		explanation:
			'This episode focuses on important character development relevant to canon. Recommended to watch.'
	},
	{
		number: 150,
		title: 'The Forbidden Jutsu Released',
		type: 'Filler',
		explanation:
			'Continuation of the Six-Tails arc. The filler arc concludes with links to canon. Recommended to watch.'
	},
	{
		number: 151,
		title: 'Master and Student',
		type: 'Filler',
		explanation:
			"The final episode of the Six-Tails Unleashed arc. It's important to watch for its relevance to canon."
	},
	{
		number: 152,
		title: 'Somber News',
		type: 'Canon'
	},
	{
		number: 153,
		title: "Following the Master's Shadow",
		type: 'Canon'
	},
	{
		number: 154,
		title: 'Decryption',
		type: 'Canon'
	},
	{
		number: 155,
		title: 'The First Challenge',
		type: 'Canon'
	},
	{
		number: 156,
		title: 'Surpassing the Master',
		type: 'Canon'
	},
	{
		number: 157,
		title: 'Assault on the Leaf Village',
		type: 'Canon'
	},
	{
		number: 158,
		title: 'Power to Believe',
		type: 'Canon'
	},
	{
		number: 159,
		title: 'Pain vs. Kakashi',
		type: 'Canon'
	},
	{
		number: 160,
		title: 'Mystery of Pain',
		type: 'Canon'
	},
	{
		number: 161,
		title: 'Surname is Sarutobi! Given Name, Konohamaru!',
		type: 'Canon'
	},
	{
		number: 162,
		title: 'Pain to the World',
		type: 'Canon'
	},
	{
		number: 163,
		title: 'Explosion! Sage Mode',
		type: 'Canon'
	},
	{
		number: 164,
		title: 'Danger! Sage Mode Limit Reached',
		type: 'Canon'
	},
	{
		number: 165,
		title: 'Nine-Tails, Captured!',
		type: 'Canon'
	},
	{
		number: 166,
		title: 'Confession',
		type: 'Canon'
	},
	{
		number: 167,
		title: 'Planetary Devastation',
		type: 'Canon'
	},
	{
		number: 168,
		title: 'The Fourth Hokage',
		type: 'Canon'
	},
	{
		number: 169,
		title: 'The Two Students',
		type: 'Canon'
	},
	{
		number: 170,
		title: "Big Adventure! The Quest for the Fourth Hokage's Legacy - Part 1",
		type: 'Filler',
		explanation:
			'This is a fun but skippable filler episode with no significant impact on the main story.'
	},
	{
		number: 171,
		title: "Big Adventure! The Quest for the Fourth Hokage's Legacy - Part 2",
		type: 'Filler',
		explanation: 'The continuation of the skippable filler arc with no relevance to the main plot.'
	},
	{
		number: 172,
		title: 'Meeting',
		type: 'Canon'
	},
	{
		number: 173,
		title: 'Origin of Pain',
		type: 'Canon'
	},
	{
		number: 174,
		title: 'Tale of Naruto Uzumaki',
		type: 'Canon'
	},
	{
		number: 175,
		title: 'Hero of the Hidden Leaf',
		type: 'Canon'
	},
	{
		number: 176,
		title: 'Rookie Instructor Iruka',
		type: 'Filler',
		explanation:
			"This episode focuses on Iruka's early experiences as an instructor. It offers some character development but is skippable."
	},
	{
		number: 177,
		title: "Iruka's Ordeal",
		type: 'Filler',
		explanation:
			"Continuation of Iruka’s journey as an instructor. Filler episode that can be skipped unless you're interested in his character."
	},
	{
		number: 178,
		title: "Iruka's Decision",
		type: 'Filler',
		explanation:
			"Completes the small filler arc about Iruka. Skippable unless you're interested in his character."
	},
	{
		number: 179,
		title: 'Kakashi Hatake, The Jonin in Charge',
		type: 'Filler',
		explanation: "A filler episode focusing on Kakashi's role as a Jonin leader. Skippable."
	},
	{
		number: 180,
		title: "Inari's Courage Put to the Test",
		type: 'Filler',
		explanation:
			"This episode revisits Inari from the original Naruto series. It's a well-loved filler episode and highly recommended."
	},
	{
		number: 181,
		title: "Naruto's School of Revenge",
		type: 'Filler',
		explanation: "A skippable filler episode that doesn't add anything significant to the story."
	},
	{
		number: 182,
		title: "Gaara's Bond",
		type: 'Filler',
		explanation:
			'A filler episode focusing on Gaara’s bond with his people. Skippable unless you are a fan of his character.'
	},
	{
		number: 183,
		title: 'Naruto: Outbreak',
		type: 'Filler',
		explanation: 'A skippable filler episode with no significant impact on the main plot.'
	},
	{
		number: 184,
		title: 'Deploy! Team Tenten',
		type: 'Filler',
		explanation:
			'A filler episode centered on Tenten’s team. Skippable unless you are a fan of her character.'
	},
	{
		number: 185,
		title: 'Animal District',
		type: 'Filler',
		explanation:
			'A filler episode with a lighthearted tone. It can be skipped without missing anything from the main plot.'
	},
	{
		number: 186,
		title: 'Ah, the Medicine of Youth',
		type: 'Filler',
		explanation: 'This episode offers comedic relief focusing on Guy and Lee. Skippable.'
	},
	{
		number: 187,
		title: 'Gutsy Master and Student: The Training',
		type: 'Filler',
		explanation: 'A filler episode that doesn’t contribute much to the main story. Skippable.'
	},
	{
		number: 188,
		title: 'Record of the Gutsy Ninja Master and Student',
		type: 'Filler',
		explanation: 'A continuation of the previous filler episode. Skippable.'
	},
	{
		number: 189,
		title: 'Sasuke’s Paw Encyclopaedia',
		type: 'Filler',
		explanation: 'A lighthearted filler focusing on Sasuke’s journey. Skippable.'
	},
	{
		number: 190,
		title: 'Naruto and the Old Soldier',
		type: 'Filler',
		explanation: "A filler episode that doesn't contribute to the main story. Skippable."
	},
	{
		number: 191,
		title: 'Kakashi Love Song',
		type: 'Filler',
		explanation:
			'A filler episode that gives insight into Kakashi’s character. Recommended if you like Kakashi, but optional.'
	},
	{
		number: 192,
		title: 'Neji Chronicles',
		type: 'Filler',
		explanation:
			"A filler episode focused on Neji’s past. Skippable unless you're a fan of his character."
	},
	{
		number: 193,
		title: 'The Man Who Died Twice',
		type: 'Filler',
		explanation: 'A filler episode with no significance to the main plot. Skippable.'
	},
	{
		number: 194,
		title: 'The Worst Three-Legged Race',
		type: 'Filler',
		explanation:
			'A well-liked and comedic filler episode. Highly recommended for its entertainment value.'
	},
	{
		number: 195,
		title: "Team 10's Teamwork",
		type: 'Filler',
		explanation:
			'A filler episode that focuses on Team 10. Skippable unless you are a fan of this team.'
	},
	{
		number: 196,
		title: 'Drive Towards Darkness',
		type: 'Filler',
		explanation:
			'A filler episode that offers background for the main story. Skippable, but some fans may enjoy the setup.'
	},
	{
		number: 197,
		title: 'The Sixth Hokage Danzo',
		type: 'Canon'
	},
	{
		number: 198,
		title: "Five Kage Summit's Eve",
		type: 'Canon'
	},
	{
		number: 199,
		title: 'Enter the Five Kage!',
		type: 'Canon'
	},
	{
		number: 200,
		title: "Naruto's Plea",
		type: 'Canon'
	},
	{
		number: 201,
		title: 'Painful Decision',
		type: 'Canon'
	},
	{
		number: 202,
		title: 'Racing Lightning',
		type: 'Canon'
	},
	{
		number: 203,
		title: "Sasuke's Ninja Way",
		type: 'Canon'
	},
	{
		number: 204,
		title: 'Power of the Five Kage',
		type: 'Canon'
	},
	{
		number: 205,
		title: 'Declaration of War',
		type: 'Canon'
	},
	{
		number: 206,
		title: "Sakura's Feelings",
		type: 'Canon'
	},
	{
		number: 207,
		title: 'The Tailed Beast vs. The Tailless Tailed Beast',
		type: 'Canon'
	},
	{
		number: 208,
		title: "As One's Friend",
		type: 'Canon'
	},
	{
		number: 209,
		title: "Danzo's Right Arm",
		type: 'Canon'
	},
	{
		number: 210,
		title: 'The Forbidden Visual Jutsu',
		type: 'Canon'
	},
	{
		number: 211,
		title: 'Danzo Shimura',
		type: 'Canon'
	},
	{
		number: 212,
		title: "Sakura's Resolve",
		type: 'Canon'
	},
	{
		number: 213,
		title: 'Lost Bonds',
		type: 'Canon'
	},
	{
		number: 214,
		title: 'The Burden',
		type: 'Canon'
	},
	{
		number: 215,
		title: 'Two Fates',
		type: 'Canon'
	},
	{
		number: 216,
		title: 'High-Level Shinobi',
		type: 'Canon'
	},
	{
		number: 217,
		title: 'The Infiltrator',
		type: 'Canon'
	},
	{
		number: 218,
		title: 'The Five Great Nations Mobilize',
		type: 'Canon'
	},
	{
		number: 219,
		title: 'Kakashi Hatake, the Hokage',
		type: 'Canon'
	},
	{
		number: 220,
		title: 'Prophecy of the Great Lord Elder',
		type: 'Canon'
	},
	{
		number: 221,
		title: 'Storage',
		type: 'Canon'
	},
	{
		number: 222,
		title: "The Five Kage's Decision",
		type: 'Canon'
	},
	{
		number: 223,
		title: 'The Young Man and the Sea',
		type: 'Filler',
		explanation: 'A filler episode focusing on a minor character’s adventure. Skippable.'
	},
	{
		number: 224,
		title: 'The Ninja of Benisu',
		type: 'Filler',
		explanation: 'A light-hearted filler episode. Skippable.'
	},
	{
		number: 225,
		title: 'The Cursed Ghost Ship',
		type: 'Filler',
		explanation:
			'A filler episode with a ghost ship plot. Skippable unless you enjoy a spooky theme.'
	},
	{
		number: 226,
		title: 'The Revived Beasts',
		type: 'Filler',
		explanation: 'Another filler episode without any major significance. Skippable.'
	},
	{
		number: 227,
		title: 'The Forgotten Island',
		type: 'Filler',
		explanation: 'A filler episode featuring an island adventure. Skippable.'
	},
	{
		number: 228,
		title: 'Fight! Rock Lee!',
		type: 'Filler',
		explanation: 'A filler episode featuring Rock Lee. Skippable unless you are a fan of Rock Lee.'
	},
	{
		number: 229,
		title: 'Eat or Die! Mushrooms from Hell',
		type: 'Filler',
		explanation: 'A comedic filler episode. Skippable unless you want some humor in the series.'
	},
	{
		number: 230,
		title: 'Revenge of the Shadow Clones',
		type: 'Filler',
		explanation: 'A filler episode about Naruto’s shadow clones. Skippable.'
	},
	{
		number: 231,
		title: 'The Closed Route',
		type: 'Filler',
		explanation: "A filler episode that doesn't impact the main story. Skippable."
	},
	{
		number: 232,
		title: "The Girls' Get-Together",
		type: 'Filler',
		explanation: 'A light-hearted filler featuring the female ninjas. Skippable.'
	},
	{
		number: 233,
		title: "Naruto's Imposter",
		type: 'Filler',
		explanation: 'A filler episode involving an imposter pretending to be Naruto. Skippable.'
	},
	{
		number: 234,
		title: "Naruto's Favorite Pupil",
		type: 'Filler',
		explanation:
			'A fun and well-loved filler episode. Highly recommended for its entertainment value.'
	},
	{
		number: 235,
		title: 'The Kunoichi of Nadeshiko Village',
		type: 'Filler',
		explanation: 'A filler episode focused on a village of kunoichi. Skippable.'
	},
	{
		number: 236,
		title: 'Friends You Can Count On',
		type: 'Filler',
		explanation: 'A filler episode centered around friendship. Skippable.'
	},
	{
		number: 237,
		title: 'Ah, My Hero Lady Tsunade!',
		type: 'Filler',
		explanation: 'A filler episode focused on Tsunade. Skippable.'
	},
	{
		number: 238,
		title: "Sai's Day Off",
		type: 'Filler',
		explanation: 'A filler episode focused on Sai’s personal time. Skippable.'
	},
	{
		number: 239,
		title: 'The Legendary Ino–Shika–Cho',
		type: 'Filler',
		explanation: 'A filler episode that focuses on the next generation of Ino–Shika–Cho. Skippable.'
	},
	{
		number: 240,
		title: "Kiba's Determination",
		type: 'Filler',
		explanation: "A filler episode about Kiba’s growth. Recommended if you're a fan of Kiba."
	},
	{
		number: 241,
		title: 'Kakashi, My Eternal Rival!',
		type: 'Filler',
		explanation:
			'A filler episode that showcases the rivalry between Kakashi and Guy. Skippable unless you enjoy their rivalry.'
	},
	{
		number: 242,
		title: "Naruto's Vow",
		type: 'Filler',
		explanation: 'A filler episode transitioning to the next arc. Skippable.'
	},
	{
		number: 243,
		title: 'Land Ahoy! Is This the Island of Paradise?',
		type: 'Canon'
	},
	{
		number: 244,
		title: 'Killer Bee and Motoi',
		type: 'Canon'
	},
	{
		number: 245,
		title: 'The Next Challenge! Naruto vs. Nine Tails',
		type: 'Canon'
	},
	{
		number: 246,
		title: 'The Orange Spark',
		type: 'Canon'
	},
	{
		number: 247,
		title: 'Target: Nine Tails',
		type: 'Canon'
	},
	{
		number: 248,
		title: "The Fourth Hokage's Death Match",
		type: 'Canon'
	},
	{
		number: 249,
		title: 'Thank You',
		type: 'Canon'
	},
	{
		number: 250,
		title: 'Battle in Paradise! The Odd Beast vs. The Monster!',
		type: 'Canon'
	},
	{
		number: 251,
		title: 'The Man Named Kisame',
		type: 'Canon'
	},
	{
		number: 252,
		title: 'The Angelic Herald of Death',
		type: 'Canon'
	},
	{
		number: 253,
		title: 'The Bridge to Peace',
		type: 'Canon'
	},
	{
		number: 254,
		title: 'The Super Secret S-Rank Mission',
		type: 'Canon'
	},
	{
		number: 255,
		title: 'The Artist Returns',
		type: 'Canon'
	},
	{
		number: 256,
		title: 'Assemble! Allied Shinobi Forces!',
		type: 'Canon'
	},
	{
		number: 257,
		title: 'Meeting',
		type: 'Filler',
		explanation:
			"Begins a flashback arc, skippable unless you're interested in revisiting old scenes."
	},
	{
		number: 258,
		title: 'Rivals',
		type: 'Filler',
		explanation: 'A continuation of the flashback arc. Skippable.'
	},
	{
		number: 259,
		title: 'Rift',
		type: 'Filler',
		explanation: 'Another flashback episode. Skippable.'
	},
	{
		number: 260,
		title: 'Parting',
		type: 'Filler',
		explanation: 'Final episode in the flashback arc. Skippable.'
	},
	{
		number: 261,
		title: 'For My Friend',
		type: 'Canon'
	},
	{
		number: 262,
		title: 'War Begins',
		type: 'Canon'
	},
	{
		number: 263,
		title: 'Sai and Shin',
		type: 'Canon'
	},
	{
		number: 264,
		title: 'Secrets of the Reanimation Jutsu',
		type: 'Canon'
	},
	{
		number: 265,
		title: 'An Old Nemesis Returns',
		type: 'Canon'
	},
	{
		number: 266,
		title: 'The First and Last Opponent',
		type: 'Canon'
	},
	{
		number: 267,
		title: 'The Brilliant Military Advisor of the Hidden Leaf',
		type: 'Canon'
	},
	{
		number: 268,
		title: 'Battleground!',
		type: 'Canon'
	},
	{
		number: 269,
		title: 'Forbidden Words',
		type: 'Canon'
	},
	{
		number: 270,
		title: 'Golden Bonds',
		type: 'Canon'
	},
	{
		number: 271,
		title: 'Road to Sakura',
		type: 'Filler',
		explanation: 'A filler episode featuring Sakura. Skippable unless you enjoy side stories.'
	},
	{
		number: 272,
		title: 'Mifune vs. Hanzo',
		type: 'Canon'
	},
	{
		number: 273,
		title: 'True Kindness',
		type: 'Canon'
	},
	{
		number: 274,
		title: 'The Complete Ino–Shika–Cho Formation!',
		type: 'Canon'
	},
	{
		number: 275,
		title: 'A Message from the Heart',
		type: 'Canon'
	},
	{
		number: 276,
		title: 'Attack of the Gedo Statue',
		type: 'Canon'
	},
	{
		number: 277,
		title: 'Unison Sign',
		type: 'Canon'
	},
	{
		number: 278,
		title: 'Medic Ninja in Danger',
		type: 'Canon'
	},
	{
		number: 279,
		title: "The White Zetsu's Trap",
		type: 'Filler',
		explanation:
			'A filler episode featuring a White Zetsu trap. Skippable unless you’re following the side story.'
	},
	{
		number: 280,
		title: 'Aesthetics of an Artist',
		type: 'Filler',
		explanation: 'A filler episode focused on Deidara’s clone. Skippable.'
	},
	{
		number: 281,
		title: 'The Allied Mom Force!!',
		type: 'Filler',
		explanation: 'A comedic filler episode involving the ninja moms. Skippable.'
	},
	{
		number: 282,
		title: 'The Secret Origin of the Ultimate Tag Team',
		type: 'Canon'
	},
	{
		number: 283,
		title: 'Two Suns',
		type: 'Canon'
	},
	{
		number: 284,
		title: 'The Helmet Splitter: Jinin Akebino!',
		type: 'Filler',
		explanation:
			'Filler but includes some background on the Seven Ninja Swordsmen. Recommended to watch.'
	},
	{
		number: 285,
		title: 'User of the Scorch Style: Pakura of the Sand!',
		type: 'Filler',
		explanation:
			"Filler with some connections to the main story, expanding Pakura's backstory. Recommended."
	},
	{
		number: 286,
		title: "Things You Can't Get Back",
		type: 'Filler',
		explanation:
			'A filler episode with minor story relevance. Skippable unless you are interested in the Seven Ninja Swordsmen arc.'
	},
	{
		number: 287,
		title: 'One Worth Betting On',
		type: 'Filler',
		explanation: 'Another filler episode in the Seven Ninja Swordsmen arc. Skippable.'
	},
	{
		number: 288,
		title: 'Danger: Jinpachi and Kushimaru!',
		type: 'Filler',
		explanation:
			'Continues the filler arc but includes some details on the Seven Ninja Swordsmen. Recommended for lore.'
	},
	{
		number: 289,
		title: 'The Lightning Blade: Ameyuri Ringo!',
		type: 'Filler',
		explanation:
			'Another filler about the Seven Ninja Swordsmen. Recommended for some plot connections.'
	},
	{
		number: 290,
		title: 'Power - Episode 1',
		type: 'Filler',
		explanation:
			"The start of the 'Power' arc, which is generally considered high-quality filler with great production. Highly recommended."
	},
	{
		number: 291,
		title: 'Power - Episode 2',
		type: 'Filler',
		explanation: "Continuation of the 'Power' arc. Highly recommended for its engaging storyline."
	},
	{
		number: 292,
		title: 'Power - Episode 3',
		type: 'Filler',
		explanation: "Another part of the well-received 'Power' arc. A must-watch."
	},
	{
		number: 293,
		title: 'Power - Episode 4',
		type: 'Filler',
		explanation: "Continues the 'Power' arc with further development. Highly recommended."
	},
	{
		number: 294,
		title: 'Power - Episode 5',
		type: 'Filler',
		explanation:
			"The penultimate episode of the 'Power' arc. Highly recommended due to its high production value."
	},
	{
		number: 295,
		title: 'Power - Final Episode',
		type: 'Filler',
		explanation: "The conclusion of the 'Power' arc, which is highly praised. A must-watch filler."
	},
	{
		number: 296,
		title: 'Naruto Enters the Battle',
		type: 'Canon'
	},
	{
		number: 297,
		title: 'A Father’s Hope, A Mother’s Love',
		type: 'Canon'
	},
	{
		number: 298,
		title: 'Contact! Naruto vs. Itachi',
		type: 'Canon'
	},
	{
		number: 299,
		title: 'The Acknowledged One',
		type: 'Canon'
	},
	{
		number: 300,
		title: 'The Mizukage, the Giant Clam, and the Mirage',
		type: 'Canon'
	},
	{
		number: 301,
		title: 'Paradox',
		type: 'Canon'
	},
	{
		number: 302,
		title: 'Terror: The Steam Imp',
		type: 'Canon'
	},
	{
		number: 303,
		title: 'Ghosts from the Past',
		type: 'Filler',
		explanation:
			"Part of the 'Chikara' arc, this filler episode introduces a new conflict unrelated to the main story. It explores haunting memories but can be skipped."
	},
	{
		number: 304,
		title: 'The Underworld Transfer Jutsu',
		type: 'Filler',
		explanation:
			"Part of the 'Chikara' arc, this episode focuses on a new jutsu, but has no connection to the core plot. Skippable."
	},
	{
		number: 305,
		title: 'The Vengeful',
		type: 'Filler',
		explanation:
			"Part of the 'Chikara' arc, this episode wraps up the mini-arc with unresolved grudges, yet it remains unrelated to the main story."
	},
	{
		number: 306,
		title: 'The Heart’s Eye',
		type: 'Filler',
		explanation: 'Focuses on Hinata’s background. Skippable unless you’re a fan of her character.'
	},
	{
		number: 307,
		title: 'Fade Into the Moonlight',
		type: 'Filler',
		explanation:
			"Start of the 'Warship Island' filler arc, which follows a mission to protect a group of refugees from pirates on a mysterious island. Can be skipped."
	},
	{
		number: 308,
		title: 'Night of the Crescent Moon',
		type: 'Filler',
		explanation: "Another episode from the 'Warship Island' filler arc. Skippable."
	},
	{
		number: 309,
		title: 'An A-Rank Mission: The Contest',
		type: 'Filler',
		explanation: 'A filler episode about a contest mission. Skippable.'
	},
	{
		number: 310,
		title: 'The Fallen Castle',
		type: 'Filler',
		explanation: 'A skippable filler episode without much impact on the main plot.'
	},
	{
		number: 311,
		title: 'Prologue of Road to Ninja',
		type: 'Filler',
		explanation:
			"A must-watch filler that serves as a prologue to the 'Road to Ninja' movie. Highly recommended."
	},
	{
		number: 312,
		title: 'The Old Master and the Dragon’s Eye',
		type: 'Filler',
		explanation: 'A filler episode with no major relevance to the main plot. Skippable.'
	},
	{
		number: 313,
		title: 'Rain Followed by Snow, with Some Lightning',
		type: 'Filler',
		explanation: 'Another skippable filler episode with little importance to the story.'
	},
	{
		number: 314,
		title: 'The Sad Sun Shower',
		type: 'Filler',
		explanation: 'A filler episode focused on side characters. Skippable.'
	},
	{
		number: 315,
		title: 'Lingering Snow',
		type: 'Filler',
		explanation: 'Another filler episode with minimal relevance to the main plot. Skippable.'
	},
	{
		number: 316,
		title: 'The Reanimated Allied Forces',
		type: 'Filler',
		explanation: 'A filler episode focused on the reanimated forces. Skippable.'
	},
	{
		number: 317,
		title: 'Shino vs. Torune',
		type: 'Filler',
		explanation: "A filler episode. Watch if you're a fan of Shino."
	},
	{
		number: 318,
		title: 'A Hole in the Heart: The Other Jinchuriki',
		type: 'Filler',
		explanation:
			"A filler episode involving the Jinchuriki. Skippable unless you're following the filler arc."
	},
	{
		number: 319,
		title: 'The Soul Living Inside the Puppet',
		type: 'Filler',
		explanation: 'A filler episode with minimal relevance to the main plot. Skippable.'
	},
	{
		number: 320,
		title: 'Run, Omoi!',
		type: 'Filler',
		explanation: 'A filler episode with no significant impact on the main plot. Skippable.'
	},
	{
		number: 321,
		title: 'Reinforcements Arrive',
		type: 'Canon'
	},
	{
		number: 322,
		title: 'Madara Uchiha',
		type: 'Canon'
	},
	{
		number: 323,
		title: 'The Five Kage Assemble',
		type: 'Canon'
	},
	{
		number: 324,
		title: 'The Unbreakable Mask and the Shattered Bubble',
		type: 'Canon'
	},
	{
		number: 325,
		title: 'Jinchuriki vs. Jinchuriki!',
		type: 'Canon'
	},
	{
		number: 326,
		title: 'Four Tails, the King of Sage Monkeys',
		type: 'Canon'
	},
	{
		number: 327,
		title: 'Nine Tails',
		type: 'Canon'
	},
	{
		number: 328,
		title: 'Kurama',
		type: 'Canon'
	},
	{
		number: 329,
		title: 'Two-Man Team',
		type: 'Canon'
	},
	{
		number: 330,
		title: 'The Promise of Victory',
		type: 'Canon'
	},
	{
		number: 331,
		title: 'Eyes That See in the Dark',
		type: 'Canon'
	},
	{
		number: 332,
		title: 'A Will of Stone',
		type: 'Canon'
	},
	{
		number: 333,
		title: 'The Risks of the Reanimation Jutsu',
		type: 'Canon'
	},
	{
		number: 334,
		title: 'Sibling Tag Team',
		type: 'Canon'
	},
	{
		number: 335,
		title: 'To Each Their Own Leaf',
		type: 'Canon'
	},
	{
		number: 336,
		title: 'Kabuto Yakushi',
		type: 'Canon'
	},
	{
		number: 337,
		title: 'The Izanami Activated',
		type: 'Canon'
	},
	{
		number: 338,
		title: 'Izanagi and Izanami',
		type: 'Canon'
	},
	{
		number: 339,
		title: 'I Will Love You Always',
		type: 'Canon'
	},
	{
		number: 340,
		title: 'Reanimation Jutsu: Release!',
		type: 'Canon'
	},
	{
		number: 341,
		title: "Orochimaru's Return",
		type: 'Canon'
	},
	{
		number: 342,
		title: 'Secret of the Transportation Technique',
		type: 'Canon'
	},
	{
		number: 343,
		title: 'Who Are You?',
		type: 'Canon'
	},
	{
		number: 344,
		title: 'Obito and Madara',
		type: 'Canon'
	},
	{
		number: 345,
		title: "I'm in Hell",
		type: 'Canon'
	},
	{
		number: 346,
		title: 'World of Dreams',
		type: 'Canon'
	},
	{
		number: 347,
		title: 'Creeping Shadow',
		type: 'Filler',
		explanation:
			'Part of the Kakashi ANBU arc. Recommended for its exploration of the origins of the Akatsuki and deeper look into Kakashi’s past.'
	},
	{
		number: 348,
		title: 'The New Akatsuki',
		type: 'Filler',
		explanation:
			'Continues the Kakashi ANBU arc, focusing on the formation of the Akatsuki. Provides essential backstory for the organization.'
	},
	{
		number: 349,
		title: 'A Mask That Hides the Heart',
		type: 'Filler',
		explanation:
			'The Kakashi ANBU arc dives into Kakashi’s emotional development and his time as part of the ANBU. Recommended for character depth.'
	},
	{
		number: 350,
		title: "Minato's Death",
		type: 'Filler',
		explanation:
			'Continues the Kakashi ANBU arc, giving insight into Minato’s role and Kakashi’s struggle. Adds significant depth to Kakashi’s story.'
	},
	{
		number: 351,
		title: "Hashirama's Cells",
		type: 'Filler',
		explanation:
			'Explores the significance of Hashirama’s cells and their impact on various characters. Recommended for fans interested in deeper lore.'
	},
	{
		number: 352,
		title: 'The Rogue Ninja Orochimaru',
		type: 'Filler',
		explanation:
			'Focuses on Orochimaru’s defection and experiments, offering important backstory for this character. A key episode in the Kakashi ANBU arc.'
	},
	{
		number: 353,
		title: "Orochimaru's Test Subject",
		type: 'Filler',
		explanation:
			'Delves further into Orochimaru’s unethical experiments and their effects on the village. Recommended for its exploration of the plot.'
	},
	{
		number: 354,
		title: 'Their Own Paths',
		type: 'Filler',
		explanation:
			'Continues the Kakashi ANBU arc, showcasing various characters’ decisions and how they shape their futures. Recommended for character growth.'
	},
	{
		number: 355,
		title: 'The Targeted Sharingan',
		type: 'Filler',
		explanation:
			'Part of the Kakashi ANBU arc, focusing on the importance of Kakashi’s Sharingan. Recommended for fans of Kakashi’s character development.'
	},
	{
		number: 356,
		title: 'A Shinobi of the Leaf',
		type: 'Filler',
		explanation:
			'Explores Kakashi’s loyalty to the village and his personal struggles as a shinobi. Recommended for further insight into his character.'
	},
	{
		number: 357,
		title: 'An Uchiha ANBU',
		type: 'Filler',
		explanation:
			'Focuses on Itachi’s role in the ANBU and his relationship with Kakashi. Recommended for Itachi fans and to understand his motivations.'
	},
	{
		number: 358,
		title: "Coup d'État",
		type: 'Filler',
		explanation:
			"Covers the buildup to the Uchiha clan's coup. Provides essential background on Itachi’s decisions and the tension within the village."
	},
	{
		number: 359,
		title: 'The Night of the Tragedy',
		type: 'Filler',
		explanation:
			'Depicts the fateful night of the Uchiha massacre from Itachi’s perspective. Recommended for those following Itachi’s story arc.'
	},
	{
		number: 360,
		title: 'Jōnin Leader',
		type: 'Filler',
		explanation:
			'Shows Kakashi’s promotion to Jonin and the formation of Team 7. Recommended for fans of Kakashi’s leadership and the team’s beginnings.'
	},
	{
		number: 361,
		title: 'Team 7',
		type: 'Filler',
		explanation:
			'Concludes the Kakashi ANBU arc by setting the stage for Team 7’s formation. A recommended episode for fans of the team’s origin story.'
	},
	{
		number: 362,
		title: 'Kakashi’s Resolve',
		type: 'Canon'
	},
	{
		number: 363,
		title: 'The Allied Shinobi Forces Jutsu',
		type: 'Canon'
	},
	{
		number: 364,
		title: 'The Ties That Bind',
		type: 'Canon'
	},
	{
		number: 365,
		title: 'Those Who Dance in the Shadows',
		type: 'Canon'
	},
	{
		number: 366,
		title: 'The All-Knowing',
		type: 'Canon'
	},
	{
		number: 367,
		title: 'Hashirama and Madara',
		type: 'Canon'
	},
	{
		number: 368,
		title: 'The Era of Warring States',
		type: 'Canon'
	},
	{
		number: 369,
		title: 'My True Dream',
		type: 'Canon'
	},
	{
		number: 370,
		title: 'Sasuke’s Answer',
		type: 'Canon'
	},
	{
		number: 371,
		title: 'Hole',
		type: 'Canon'
	},
	{
		number: 372,
		title: 'Something to Fill the Hole',
		type: 'Canon'
	},
	{
		number: 373,
		title: 'Team 7, Assemble!',
		type: 'Canon'
	},
	{
		number: 374,
		title: 'The New Three-Way Deadlock',
		type: 'Canon'
	},
	{
		number: 375,
		title: 'Kakashi vs. Obito',
		type: 'Canon'
	},
	{
		number: 376,
		title: 'The Directive to Take the Nine Tails',
		type: 'Filler',
		explanation: 'A filler episode that adds no major story elements. Skippable.'
	},
	{
		number: 377,
		title: 'Naruto vs. Mecha Naruto',
		type: 'Filler',
		explanation: 'A humorous filler episode with no impact on the plot. Skippable.'
	},
	{
		number: 378,
		title: "The Ten Tails' Jinchuriki",
		type: 'Canon'
	},
	{
		number: 379,
		title: 'An Opening',
		type: 'Canon'
	},
	{
		number: 380,
		title: 'The Day Naruto Was Born',
		type: 'Canon'
	},
	{
		number: 381,
		title: 'The Divine Tree',
		type: 'Canon'
	},
	{
		number: 382,
		title: 'A Shinobi’s Dream',
		type: 'Canon'
	},
	{
		number: 383,
		title: 'Pursuing Hope',
		type: 'Canon'
	},
	{
		number: 384,
		title: 'A Heart Filled With Comrades',
		type: 'Canon'
	},
	{
		number: 385,
		title: 'Obito Uchiha',
		type: 'Canon'
	},
	{
		number: 386,
		title: 'I’m Always Watching',
		type: 'Canon'
	},
	{
		number: 387,
		title: 'The Promise That Was Kept',
		type: 'Canon'
	},
	{
		number: 388,
		title: 'My First Friend',
		type: 'Filler',
		explanation: 'An emotional filler episode with no major plot relevance. Skippable.'
	},
	{
		number: 389,
		title: 'The Adored Elder Sister',
		type: 'Filler',
		explanation: 'Focuses on Hinata and Hanabi’s relationship. Recommended for character depth.'
	},
	{
		number: 390,
		title: 'Hanabi’s Decision',
		type: 'Filler',
		explanation:
			'Continues exploring the bond between Hinata and Hanabi. Recommended for Hyuga fans.'
	},
	{
		number: 391,
		title: 'Madara Uchiha Arises',
		type: 'Canon'
	},
	{
		number: 392,
		title: 'The Hidden Heart',
		type: 'Canon'
	},
	{
		number: 393,
		title: 'A True Ending',
		type: 'Canon'
	},
	{
		number: 394,
		title: 'The New Chunin Exams',
		type: 'Filler',
		explanation: "Begins the filler Chunin Exams arc. Skippable unless you're following this arc."
	},
	{
		number: 395,
		title: 'The Chunin Exams Begin!',
		type: 'Filler',
		explanation: 'Continuation of the Chunin Exams filler arc. Skippable.'
	},
	{
		number: 396,
		title: 'The Three Questions',
		type: 'Filler',
		explanation:
			"Another filler episode within the Chunin Exams arc. Skippable unless you're interested in the exam format."
	},
	{
		number: 397,
		title: 'One Worthy as a Leader',
		type: 'Filler',
		explanation: 'Part of the Chunin Exams filler arc. Skippable unless following the arc.'
	},
	{
		number: 398,
		title: 'The Night Before the Second Exam',
		type: 'Filler',
		explanation:
			'A filler episode focused on preparations for the next stage of the Chunin Exams. Skippable.'
	},
	{
		number: 399,
		title: 'Demon Desert Survival',
		type: 'Filler',
		explanation: "Continues the Chunin Exams arc. Skippable unless you're following the arc."
	},
	{
		number: 400,
		title: 'As a Taijutsu User',
		type: 'Filler',
		explanation: 'Filler episode in the Chunin Exams filler arc focused on taijutsu. Skippable.'
	},
	{
		number: 401,
		title: 'The Ultimate',
		type: 'Filler',
		explanation: 'Another episode in the Chunin Exams filler arc. Skippable.'
	},
	{
		number: 402,
		title: 'Escape vs. Pursuit',
		type: 'Filler',
		explanation: 'Part of the Chunin Exams filler arc. Skippable.'
	},
	{
		number: 403,
		title: 'Unwavering Guts',
		type: 'Filler',
		explanation: 'Continues the Chunin Exams arc. Skippable.'
	},
	{
		number: 404,
		title: 'Tenten’s Troubles',
		type: 'Filler',
		explanation: "Focuses on Tenten. Skippable unless you're a fan of her character."
	},
	{
		number: 405,
		title: 'The Imprisoned Pair',
		type: 'Filler',
		explanation: 'Another episode in the Chunin Exams filler arc. Skippable.'
	},
	{
		number: 406,
		title: 'The Place Where I Belong',
		type: 'Filler',
		explanation: 'Episode in the Chunin Exams filler arc focused on teamwork. Skippable.'
	},
	{
		number: 407,
		title: 'The Yamanaka Clan: Secret Ninjutsu',
		type: 'Filler',
		explanation:
			"Episode in the Chunin Exams filler arc that explores the Yamanaka clan's secret techniques. Skippable."
	},
	{
		number: 408,
		title: 'The Cursed Puppet',
		type: 'Filler',
		explanation: "Continues the Chunin Exams arc. Skippable unless you're invested in the arc."
	},
	{
		number: 409,
		title: 'Their Backs',
		type: 'Filler',
		explanation: 'Another episode within the Chunin Exams arc. Skippable.'
	},
	{
		number: 410,
		title: 'The Hidden Plot Set Into Motion',
		type: 'Filler',
		explanation: 'A filler episode focusing on a secret plot. Skippable.'
	},
	{
		number: 411,
		title: 'The Targeted Tailed Beast',
		type: 'Filler',
		explanation:
			"Involves the tailed beasts. Skippable unless you're interested in their background."
	},
	{
		number: 412,
		title: 'Neji’s Judgment',
		type: 'Filler',
		explanation: "Focuses on Neji's decision-making. Skippable unless you're a fan of Neji."
	},
	{
		number: 413,
		title: 'Hopes Entrusted to the Future',
		type: 'Filler',
		explanation: "Concludes the Chunin Exams arc. Skippable unless you're following the arc."
	},
	{
		number: 414,
		title: 'On the Brink of Death',
		type: 'Canon'
	},
	{
		number: 415,
		title: 'The Two Mangekyo',
		type: 'Canon'
	},
	{
		number: 416,
		title: 'The Formation of Team Minato',
		type: 'Filler',
		explanation: "A filler episode focusing on Team Minato's origins. Skippable."
	},
	{
		number: 417,
		title: "You'll Be My Backup",
		type: 'Filler',
		explanation: 'A filler episode focusing on the bond between Lee and Guy.'
	},
	{
		number: 418,
		title: 'The Blue Beast vs. Six Paths Madara',
		type: 'Canon'
	},
	{
		number: 419,
		title: 'Papa’s Youth',
		type: 'Canon'
	},
	{
		number: 420,
		title: 'The Eight Inner Gates Formation',
		type: 'Canon'
	},
	{
		number: 421,
		title: 'The Sage of the Six Paths',
		type: 'Canon'
	},
	{
		number: 422,
		title: 'The Ones Who Will Inherit',
		type: 'Filler',
		explanation: 'A filler episode focusing on the legacy of past generations. Skippable.'
	},
	{
		number: 423,
		title: 'Naruto’s Rival',
		type: 'Filler',
		explanation: "Focuses on Naruto's rivalry with his peers. Skippable."
	},
	{
		number: 424,
		title: 'To Rise Up',
		type: 'Canon'
	},
	{
		number: 425,
		title: 'The Infinite Tsukuyomi',
		type: 'Canon'
	},
	{
		number: 426,
		title: 'The Infinite Dream',
		type: 'Canon'
	},
	{
		number: 427,
		title: 'To the Dreamworld',
		type: 'Filler',
		explanation:
			"Begins the Infinite Tsukuyomi dream sequence arc. Skippable unless you're curious about each character's dreams."
	},
	{
		number: 428,
		title: 'Where Tenten Belongs',
		type: 'Filler',
		explanation: "Focuses on Tenten's dream in the Infinite Tsukuyomi. Skippable."
	},
	{
		number: 429,
		title: 'Killer Bee Rappuden - Part 1',
		type: 'Filler',
		explanation:
			"A comedic episode featuring Killer Bee. Skippable unless you're a fan of Killer Bee's humor."
	},
	{
		number: 430,
		title: 'Killer Bee Rappuden - Part 2',
		type: 'Filler',
		explanation: 'Continuation of the comedic Killer Bee filler arc. Skippable.'
	},
	{
		number: 431,
		title: 'To See That Smile, Just One More Time',
		type: 'Filler',
		explanation: 'Focuses on the bond between Naruto and Sasuke. Skippable.'
	},
	{
		number: 432,
		title: 'The Loser Ninja',
		type: 'Filler',
		explanation: "Begins the 'Jiraiya Ninja Scrolls' arc. Skippable."
	},
	{
		number: 433,
		title: 'The Search Mission',
		type: 'Filler',
		explanation: "Part of the 'Jiraiya Ninja Scrolls' filler arc. Skippable."
	},
	{
		number: 434,
		title: 'Team Jiraiya',
		type: 'Filler',
		explanation: "Another filler episode from the 'Jiraiya Ninja Scrolls' arc. Skippable."
	},
	{
		number: 435,
		title: 'Order of Priority',
		type: 'Filler',
		explanation: "Continuation of the 'Jiraiya Ninja Scrolls' arc. Skippable."
	},
	{
		number: 436,
		title: 'The Masked Man',
		type: 'Filler',
		explanation: "Part of the 'Jiraiya Ninja Scrolls' arc. Skippable."
	},
	{
		number: 437,
		title: 'The Sealed Power',
		type: 'Filler',
		explanation: "Continues the 'Jiraiya Ninja Scrolls' arc. Skippable."
	},
	{
		number: 438,
		title: 'The Rules or a Comrade',
		type: 'Filler',
		explanation: "Focuses on teamwork in the 'Jiraiya Ninja Scrolls' arc. Skippable."
	},
	{
		number: 439,
		title: 'The Child of Prophecy',
		type: 'Filler',
		explanation: "Another episode in the 'Jiraiya Ninja Scrolls' arc. Skippable."
	},
	{
		number: 440,
		title: 'The Caged Bird',
		type: 'Filler',
		explanation: "Continues the 'Jiraiya Ninja Scrolls' arc. Skippable."
	},
	{
		number: 441,
		title: 'Returning Home',
		type: 'Filler',
		explanation: "Another episode in the 'Jiraiya Ninja Scrolls' arc. Skippable."
	},
	{
		number: 442,
		title: 'The Mutual Path',
		type: 'Filler',
		explanation: "Continuation of the 'Jiraiya Ninja Scrolls' arc. Skippable."
	},
	{
		number: 443,
		title: 'The Difference in Power',
		type: 'Filler',
		explanation: "Explores power dynamics within the 'Jiraiya Ninja Scrolls' arc. Skippable."
	},
	{
		number: 444,
		title: 'Leaving the Village',
		type: 'Filler',
		explanation: "Another episode in the 'Jiraiya Ninja Scrolls' arc. Skippable."
	},
	{
		number: 445,
		title: 'Pursuers',
		type: 'Filler',
		explanation: "Continuation of the 'Jiraiya Ninja Scrolls' arc. Skippable."
	},
	{
		number: 446,
		title: 'Collision',
		type: 'Filler',
		explanation: "Another episode in the 'Jiraiya Ninja Scrolls' arc. Skippable."
	},
	{
		number: 447,
		title: 'Another Moon',
		type: 'Filler',
		explanation: "Focuses on a parallel world in the 'Jiraiya Ninja Scrolls' arc. Skippable."
	},
	{
		number: 448,
		title: 'Comrade',
		type: 'Filler',
		explanation: "Continues the 'Jiraiya Ninja Scrolls' arc. Skippable."
	},
	{
		number: 449,
		title: 'The Shinobi Unite',
		type: 'Filler',
		explanation: "A filler episode in the 'Jiraiya Ninja Scrolls' arc. Skippable."
	},
	{
		number: 450,
		title: 'Rival',
		type: 'Filler',
		explanation: "The conclusion of the 'Jiraiya Ninja Scrolls' filler arc. Skippable."
	},
	{
		number: 451,
		title: "Itachi's Story - Light and Darkness: Birth and Death",
		type: 'Mixed Canon/Filler',
		explanation:
			"This episode is part of the 'Itachi Shinden' arc, focusing on Itachi's backstory, with some filler elements expanding on his early life."
	},
	{
		number: 452,
		title: "Itachi's Story - Light and Darkness: The Pain of Living",
		type: 'Mixed Canon/Filler',
		explanation:
			"Continues 'Itachi Shinden,' Adds filler scenes that provide more depth to Itachi's personal journey."
	},
	{
		number: 453,
		title: "Itachi's Story - Light and Darkness: Shisui's Request",
		type: 'Mixed Canon/Filler',
		explanation:
			'A mix of canon and filler, focusing on Itachi and his relationship with Shisui. Expands on their bond with filler content for additional context.'
	},
	{
		number: 454,
		title: "Itachi's Story - Light and Darkness: Moonlit Night",
		type: 'Mixed Canon/Filler',
		explanation:
			'This episode continues Itachi’s story, with filler content that further explores his emotional struggles and decision-making.'
	},
	{
		number: 455,
		title: "Itachi's Story - Light and Darkness: The Darkness of the Akatsuki",
		type: 'Mixed Canon/Filler',
		explanation:
			'Focuses on Itachi’s time in the Akatsuki, added filler material for narrative depth.'
	},
	{
		number: 456,
		title: "Itachi's Story - Light and Darkness: The Darkness of the Akatsuki, Part 2",
		type: 'Mixed Canon/Filler',
		explanation:
			'Further develops Itachi’s journey within the Akatsuki, mixing canon events with filler to flesh out character interactions and emotions.'
	},
	{
		number: 457,
		title: "Itachi's Story - Light and Darkness: Partner",
		type: 'Mixed Canon/Filler',
		explanation:
			'A mix of canon and filler, focusing on Itachi’s partnership in the Akatsuki. Provides additional filler scenes to emphasize his internal struggles.'
	},
	{
		number: 458,
		title: "Itachi's Story - Light and Darkness: Truth",
		type: 'Mixed Canon/Filler',
		explanation:
			"Concludes the 'Itachi Shinden' arc, blending canon moments with filler elements that wrap up Itachi’s emotional journey."
	},
	{
		number: 459,
		title: 'She of the Beginning',
		type: 'Canon'
	},
	{
		number: 460,
		title: 'Kaguya Otsutsuki',
		type: 'Mixed Canon/Filler',
		explanation:
			"Provides important backstory about Kaguya Otsutsuki. Recommended for understanding the series' lore."
	},
	{
		number: 461,
		title: 'Hagoromo and Hamura',
		type: 'Mixed Canon/Filler',
		explanation:
			'Continues the backstory of Kaguya and her children. Recommended for those interested in the origins of chakra.'
	},
	{
		number: 462,
		title: 'A Fabricated Past',
		type: 'Mixed Canon/Filler',
		explanation:
			"Further explores Kaguya's past and its impact on the main story. Recommended for lore."
	},
	{
		number: 463,
		title: 'The No. 1 Most Unpredictable Ninja',
		type: 'Canon'
	},
	{
		number: 464,
		title: 'Ninshu: The Ninja Creed',
		type: 'Filler',
		explanation:
			"Focuses on the development of Ninshu, the precursor to ninjutsu. Recommended for those interested in the series' mythology."
	},
	{
		number: 465,
		title: 'Ashura and Indra',
		type: 'Filler',
		explanation:
			'Introduces the origin of the eternal conflict between Ashura and Indra, which mirrors Naruto and Sasuke. Recommended for lore enthusiasts.'
	},
	{
		number: 466,
		title: 'The Tumultuous Journey',
		type: 'Filler',
		explanation:
			"Continues Ashura and Indra's journey, offering key insights into the origins of chakra and ninjutsu. Recommended."
	},
	{
		number: 467,
		title: 'Ashura’s Decision',
		type: 'Filler',
		explanation:
			"Focuses on Ashura's journey of growth. Important for understanding the series' core themes. Recommended."
	},
	{
		number: 468,
		title: 'The Successor',
		type: 'Filler',
		explanation:
			'Concludes the story of Ashura and Indra, highlighting the origins of many series themes. Recommended.'
	},
	{
		number: 469,
		title: 'A Special Mission',
		type: 'Mixed Canon/Filler',
		explanation:
			"A fan-favorite episode revealing Kakashi's face, adapted from a canon omake. A must-watch."
	},
	{
		number: 470,
		title: 'Connecting Thoughts',
		type: 'Canon'
	},
	{
		number: 471,
		title: 'The Two of Them… Always',
		type: 'Canon'
	},
	{
		number: 472,
		title: 'You Better...',
		type: 'Canon'
	},
	{
		number: 473,
		title: 'The Sharingan Revived',
		type: 'Canon'
	},
	{
		number: 474,
		title: 'Congratulations',
		type: 'Canon'
	},
	{
		number: 475,
		title: 'The Final Valley',
		type: 'Canon'
	},
	{
		number: 476,
		title: 'The Final Battle',
		type: 'Canon'
	},
	{
		number: 477,
		title: 'Naruto and Sasuke',
		type: 'Canon'
	},
	{
		number: 478,
		title: 'The Unison Sign',
		type: 'Canon'
	},
	{
		number: 479,
		title: 'Naruto Uzumaki!!',
		type: 'Canon'
	},
	{
		number: 480,
		title: 'Naruto and Hinata',
		type: 'Filler',
		explanation:
			"Part of the 'Childhood' arc directed by Osamu Kobayashi. Explores Naruto and Hinata's childhoods. Highly recommended for character development."
	},
	{
		number: 481,
		title: 'Sasuke and Sakura',
		type: 'Filler',
		explanation:
			"Continues the 'Childhood' arc, focusing on Sasuke and Sakura. Highly recommended for those who want deeper insight into their backstories."
	},
	{
		number: 482,
		title: 'Gaara and Shikamaru',
		type: 'Filler',
		explanation:
			"Explores the childhoods of Gaara and Shikamaru in the 'Childhood' arc. Highly recommended for fans of these characters."
	},
	{
		number: 483,
		title: 'Jiraiya and Kakashi',
		type: 'Filler',
		explanation:
			"Concludes the 'Childhood' arc with a focus on Jiraiya and Kakashi. A must-watch for those invested in these two characters."
	},
	{
		number: 484,
		title: 'Sasuke’s Story: Sunrise, Part 1: The Exploding Human',
		type: 'Canon'
	},
	{
		number: 485,
		title: 'Sasuke’s Story: Sunrise, Part 2: Colosseum',
		type: 'Canon'
	},
	{
		number: 486,
		title: 'Sasuke’s Story: Sunrise, Part 3: Fūshin',
		type: 'Canon'
	},
	{
		number: 487,
		title: 'Sasuke’s Story: Sunrise, Part 4: The Ketsuryugan',
		type: 'Canon'
	},
	{
		number: 488,
		title: 'Sasuke’s Story: Sunrise, Part 5: The Last One',
		type: 'Canon'
	},
	{
		number: 489,
		title: 'Shikamaru’s Story: A Cloud Drifting in the Silent Dark, Part 1: The State of Affairs',
		type: 'Canon'
	},
	{
		number: 490,
		title: 'Shikamaru’s Story: A Cloud Drifting in the Silent Dark, Part 2: Dark Clouds',
		type: 'Canon'
	},
	{
		number: 491,
		title: 'Shikamaru’s Story: A Cloud Drifting in the Silent Dark, Part 3: Recklessness',
		type: 'Canon'
	},
	{
		number: 492,
		title: 'Shikamaru’s Story: A Cloud Drifting in the Silent Dark, Part 4: Cloud of Suspicion',
		type: 'Canon'
	},
	{
		number: 493,
		title: 'Shikamaru’s Story: A Cloud Drifting in the Silent Dark, Part 5: Dawn',
		type: 'Canon'
	},
	{
		number: 494,
		title: 'Hidden Leaf Story, The Perfect Day for a Wedding, Part 1: Naruto’s Wedding',
		type: 'Canon'
	},
	{
		number: 495,
		title: 'Hidden Leaf Story, The Perfect Day for a Wedding, Part 2: A Full-Powered Wedding Gift',
		type: 'Canon'
	},
	{
		number: 496,
		title: 'Hidden Leaf Story, The Perfect Day for a Wedding, Part 3: Steam and Food Pills',
		type: 'Canon'
	},
	{
		number: 497,
		title: 'Hidden Leaf Story, The Perfect Day for a Wedding, Part 4: The Kazekage’s Wedding Gift',
		type: 'Canon'
	},
	{
		number: 498,
		title: 'Hidden Leaf Story, The Perfect Day for a Wedding, Part 5: The Last Mission',
		type: 'Canon'
	},
	{
		number: 499,
		title:
			'Hidden Leaf Story, The Perfect Day for a Wedding, Part 6: The Outcome of the Secret Mission',
		type: 'Canon'
	},
	{
		number: 500,
		title: 'Hidden Leaf Story, The Perfect Day for a Wedding, Part 7: The Message',
		type: 'Canon'
	}
];

export const narutoShippudenArcs: Arc[] = [
	{
		name: 'Kazekage Rescue Mission',
		episode_start: 1,
		episode_end: 32
	},
	{
		name: 'Sasuke and Sai',
		episode_start: 33,
		episode_end: 53
	},
	{
		name: 'Twelve Guardian Ninja',
		episode_start: 54,
		episode_end: 71
	},
	{
		name: 'Hidan and Kakuzu',
		episode_start: 72,
		episode_end: 88
	},
	{
		name: "Three-Tails' Appearance",
		episode_start: 89,
		episode_end: 112
	},
	{
		name: 'Itachi Pursuit Mission',
		episode_start: 113,
		episode_end: 118
	},
	{
		name: "Kakashi Chronicles ~ Boys' Life on the Battlefield",
		episode_start: 119,
		episode_end: 120
	},
	{
		name: 'Itachi Pursuit Mission (Continued)',
		episode_start: 121,
		episode_end: 126
	},
	{
		name: 'Tale of Jiraiya the Gallant',
		episode_start: 127,
		episode_end: 133
	},
	{
		name: 'Fated Battle Between Brothers',
		episode_start: 134,
		episode_end: 143
	},
	{
		name: 'Six-Tails Unleashed',
		episode_start: 144,
		episode_end: 151
	},
	{
		name: "Pain's Assault",
		episode_start: 152,
		episode_end: 175
	},
	{
		name: 'Past Arc: The Locus of Konoha',
		episode_start: 176,
		episode_end: 196
	},
	{
		name: 'Five Kage Summit',
		episode_start: 197,
		episode_end: 214
	},
	{
		name: 'Fourth Shinobi World War: Countdown',
		episode_start: 215,
		episode_end: 222
	},
	{
		name: 'Paradise Life on a Boat',
		episode_start: 223,
		episode_end: 242
	},
	{
		name: 'Fourth Shinobi World War: Confrontation',
		episode_start: 243,
		episode_end: 275
	},
	{
		name: 'Fourth Shinobi World War: Climax',
		episode_start: 276,
		episode_end: 295
	},
	{
		name: 'Power (Filler Arc)',
		episode_start: 290,
		episode_end: 295
	},
	{
		name: 'Fourth Shinobi World War: Climax (Continued)',
		episode_start: 296,
		episode_end: 320
	},
	{
		name: 'Fourth Shinobi World War: The Return of Madara',
		episode_start: 321,
		episode_end: 348
	},
	{
		name: "Kakashi's Anbu Arc: The Shinobi That Lives in the Darkness",
		episode_start: 349,
		episode_end: 361
	},
	{
		name: 'Fourth Shinobi World War: The Return of Madara (Continued)',
		episode_start: 362,
		episode_end: 375
	},
	{
		name: "The Ten-Tails' Jinchuriki",
		episode_start: 376,
		episode_end: 393
	},
	{
		name: "Birth of the Ten-Tails' Jinchuriki",
		episode_start: 394,
		episode_end: 413
	},
	{
		name: 'Kaguya Otsutsuki Strikes',
		episode_start: 414,
		episode_end: 431
	},
	{
		name: "Jiraiya's Ninja Scrolls: The Tale of Naruto the Hero",
		episode_start: 432,
		episode_end: 450
	},
	{
		name: 'Itachi Shinden Book: Light and Darkness',
		episode_start: 451,
		episode_end: 458
	},
	{
		name: 'Kaguya Otsutsuki Strikes (Continued)',
		episode_start: 459,
		episode_end: 479
	},
	{
		name: 'Sasuke Shinden: Book of Sunrise',
		episode_start: 480,
		episode_end: 488
	},
	{
		name: 'Shikamaru Hiden: A Cloud Drifting in Silent Darkness',
		episode_start: 489,
		episode_end: 493
	},
	{
		name: 'Hidden Leaf Story, The Perfect Day for a Wedding',
		episode_start: 494,
		episode_end: 500
	}
];

export const narutoShippudenQuickList: QuickList = {
	Manga_Canon_Episodes: [
		'20-23',
		'26-28',
		'29-44',
		'46-48',
		'51-53',
		'55',
		'72-88',
		'114',
		'116-126',
		'129-143',
		'152-169',
		'172-175',
		'197-212',
		'214-222',
		'243-253',
		'255-256',
		'261-270',
		'272-278',
		'282-283',
		'297-302',
		'321-323',
		'325-326',
		'329',
		'332-337',
		'339-345',
		'363-375',
		'378-384',
		'387',
		'391-393',
		'414',
		'418',
		'420-421',
		'424-425',
		'459',
		'463',
		'470',
		'473-477',
		'484-500'
	],
	Mixed_Canon_Filler_Episodes: [
		'1-19',
		'24-25',
		'45',
		'49-50',
		'54',
		'56',
		'89-90',
		'113',
		'115',
		'127-128',
		'213',
		'254',
		'296',
		'324',
		'327-328',
		'330-331',
		'338',
		'346',
		'362',
		'385-386',
		'415',
		'419',
		'426',
		'451-458',
		'460-462',
		'469',
		'471-472',
		'478-479'
	],
	Filler_Episodes: [
		'57-71',
		'91-112',
		'144-151',
		'170-171',
		'176-196',
		'223-242',
		'257-260',
		'271',
		'279-281',
		'284-295',
		'303-320',
		'347-361',
		'376-377',
		'388-390',
		'394-413',
		'416-417',
		'422-423',
		'427-450',
		'464-468',
		'480-483'
	]
};
